# Dressing Stock Register API

## Overview

The Dressing Stock Register API generates an Excel report that shows dressing stock movements by **Item Group Name** and **Item Name** over a date range. The report includes Opening Balance, Purchase, Receipt, Issue Sq Mtr, Clipping, Dyeing, Mixmatch, Edgebanding, Lipping, Redressing, Sale, and Closing Balance, with a **Total** row at the bottom.

Data is sourced from `dressing_done_items`, `dressing_done_other_details` (for receipt by dressing date), and `dressing_miss_match_data` (for the Mixmatch column).

## Endpoint

```
POST /api/V1/report/download-excel-dressing-stock-register
```

## Request Body

### Required Parameters

```json
{
  "startDate": "2025-01-01",
  "endDate": "2025-01-31"
}
```

### Optional Parameters

```json
{
  "startDate": "2025-01-01",
  "endDate": "2025-01-31",
  "filter": {
    "item_name": "ASH",
    "item_group_name": "ASH"
  }
}
```

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `startDate` | String | Yes | Start date in YYYY-MM-DD format |
| `endDate` | String | Yes | End date in YYYY-MM-DD format |
| `filter.item_name` | String | No | Filter by item name |
| `filter.item_group_name` | String | No | Filter by item group (maps to item sub-category name) |

## Response

### Success Response (200 OK)

```json
{
  "statusCode": 200,
  "success": true,
  "message": "Dressing stock register generated successfully",
  "result": "http://localhost:5000/public/upload/reports/reports2/Dressing/Dressing-Stock-Register-1738234567890.xlsx"
}
```

### Error Responses

#### 400 Bad Request - Missing Parameters

```json
{
  "statusCode": 400,
  "success": false,
  "message": "Start date and end date are required"
}
```

#### 400 Bad Request - Invalid Date Format

```json
{
  "statusCode": 400,
  "success": false,
  "message": "Invalid date format. Use YYYY-MM-DD"
}
```

#### 400 Bad Request - Invalid Date Range

```json
{
  "statusCode": 400,
  "success": false,
  "message": "Start date cannot be after end date"
}
```

#### 404 Not Found - No Data

```json
{
  "statusCode": 404,
  "success": false,
  "message": "No dressing data found for the selected period"
}
```

or when all rows are filtered out:

```json
{
  "statusCode": 404,
  "success": false,
  "message": "No dressing stock data found for the selected period"
}
```

---

## How the data is brought together

A new developer should be able to understand the report from this section without reading the controller code.

### 1. Report period

- **start**: `new Date(startDate)` at 00:00:00.000  
- **end**: `new Date(endDate)` at 23:59:59.999  
- All “in period” logic uses this inclusive range.

### 2. Which rows appear in the report

- We take **distinct (item_sub_category_name, item_name)** from the collection **dressing_done_items** (optionally filtered by `filter.item_name` and `filter.item_group_name` → `item_sub_category_name`).
- There is **exactly one report row per** such pair. If an item has no activity in the period, it still appears if it exists in dressing_done_items, unless it is dropped in step 6 below.

### 3. Collections and fields used

| Collection | Role | Key fields |
|------------|------|------------|
| **dressing_done_items** | Production items (receipt when done; issues when status/updatedAt set) | `dressing_done_other_details_id`, `item_name`, `item_sub_category_name`, `sqm`, `issue_status`, `updatedAt` |
| **dressing_done_other_details** | Session header; provides **dressing_date** for “receipt in period” | `_id`, `dressing_date` |
| **dressing_miss_match_data** | Mismatch records; drives **Mixmatch** column | `item_name`, `item_sub_category_name`, `sqm`, `dressing_date` |

- **Join**: `dressing_done_items.dressing_done_other_details_id` → `dressing_done_other_details._id` (used only to get `dressing_date` for receipt).

### 4. Per-row aggregates (for each item_sub_category_name + item_name)

All of the following are **sums of `sqm`** (or 0 if no rows), scoped to that pair.

| Quantity | Collection(s) | Filter | Meaning |
|----------|---------------|--------|---------|
| **Current available** | dressing_done_items | `issue_status` is `null` or not present | Stock still “on hand” (not yet issued). |
| **Receipt** | dressing_done_items + dressing_done_other_details | Join by `dressing_done_other_details_id` → `_id`; then `dressing_date` ∈ [start, end] | Dressing production **received** in the report period (by session date). |
| **Issue for order** | dressing_done_items | `issue_status === 'order'` and `updatedAt` ∈ [start, end] | SQM issued for order in the period. |
| **Issue for grouping** | dressing_done_items | `issue_status === 'grouping'` and `updatedAt` ∈ [start, end] | SQM issued for grouping in the period. |
| **Issue for dyeing** | dressing_done_items | `issue_status === 'smoking_dying'` and `updatedAt` ∈ [start, end] | SQM issued for smoking/dyeing in the period. |
| **Mixmatch** | dressing_miss_match_data | `dressing_date` ∈ [start, end] | Mismatch SQM in the period (separate collection). |

- **Issue Sq Mtr** (report column) = Issue for order + Issue for grouping.  
- **Dyeing** (report column) = Issue for dyeing.  
- **Issued in period** (internal) = Issue Sq Mtr + Dyeing (used only in opening balance formula).

Placeholder columns (no source yet): **Purchase**, **Clipping**, **Edgebanding**, **Lipping**, **Redressing**, **Sale** → all **0**.

### 5. Formulas (calculations)

For each (item_sub_category_name, item_name):

```
Opening Balance = max(0,  Current available  +  Issued in period  −  Receipt  )

Closing Balance = max(0,  Opening Balance  +  Purchase  +  Receipt  −  Total issues  )

where:
  Issued in period = Issue Sq Mtr + Dyeing   (from dressing_done_items only)
  Total issues     = Issue Sq Mtr + Clipping + Dyeing + Mixmatch + Edgebanding + Lipping + Redressing + Sale
```

So:

- **Opening** = what would have been “current” at start of period if we reverse the period’s receipts and add back the period’s issues (same idea as item-wise flitch).  
- **Closing** = opening + inflows (purchase + receipt) − all outflows (issue columns + sale).  
- Both are floored at 0.

### 6. Which rows are returned in the Excel

- After computing the above for every distinct (item_sub_category_name, item_name), we **drop rows where every numeric column is 0** (opening_balance, purchase, receipt, issue_sq_mtr, clipping, dyeing, mixmatch, edgebanding, lipping, redressing, sale, closing_balance).  
- If no rows remain, the API responds with **404** and message `"No dressing stock data found for the selected period"`.  
- The Excel is built from the remaining rows only.

### 7. Understanding the API response

- **200**: The report was generated. **result** is a **URL** to the Excel file (e.g. `http://localhost:5000/public/upload/reports/reports2/Dressing/Dressing-Stock-Register-<timestamp>.xlsx`). The client can GET this URL to download the file.  
- The Excel contains one sheet: title row with date range, one header row (column names), one data row per (Item Group Name, Item Name) that passed the non-zero filter, then one **Total** row (sum of each numeric column).  
- **400**: Invalid request (missing/invalid dates or start &gt; end).  
- **404**: No distinct (item_sub_category_name, item_name) in dressing_done_items, or all rows were dropped as all-zero.

---

## Report Structure

The generated Excel file has the following layout.

### Title Row

```
Dressing Stock Register - DD/MM/YYYY-DD/MM/YYYY
```

Example: `Dressing Stock Register - 01/03/2025-31/03/2025`

### Column Headers

| Column | Description |
|--------|-------------|
| Item Group Name | Item sub-category name |
| Item Name | Item name |
| Opening Balance | Stock at start of period (SQM) |
| Purchase | Purchase in period (currently 0) |
| Receipt | Dressing production receipt in period (SQM) |
| Issue Sq Mtr | Issued for order + grouping in period (SQM) |
| Clipping | Clipping in period (currently 0) |
| Dyeing | Issued for smoking/dyeing in period (SQM) |
| Mixmatch | Dressing mismatch in period (SQM) from `dressing_miss_match_data` |
| Edgebanding | Edgebanding in period (currently 0) |
| Lipping | Lipping in period (currently 0) |
| Redressing | Redressing in period (currently 0) |
| Sale | Sale in period (currently 0) |
| Closing Balance | Opening + Purchase + Receipt − all issue columns (SQM) |

### Data Rows

- One row per distinct **(Item Group Name, Item Name)** from dressing done items.
- Sorted by Item Group Name, then Item Name.
- Numeric columns use two decimal places.

### Total Row

- Last row is **Total**, with sums of all numeric columns (Opening Balance through Closing Balance).

## Implementation References

- **Controller**: `topl_backend/controllers/reports2/Dressing/dressingStockRegister.js`
- **Excel config**: `topl_backend/config/downloadExcel/reports2/Dressing/dressingStockRegister.js`
- **Routes**: `topl_backend/routes/report/reports2/Dressing/dressing.routes.js`
- **Plan** (design and implementation steps): [DRESSING_STOCK_REGISTER_PLAN.md](./DRESSING_STOCK_REGISTER_PLAN.md) in this folder.

For how data is gathered, which collections/fields are used, and the exact formulas, see **[How the data is brought together](#how-the-data-is-brought-together)** above.
