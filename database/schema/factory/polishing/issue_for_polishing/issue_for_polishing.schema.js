...
const issue_for_polishing_schema = new mongoose.Schema(
  {
    sr_no: Number,
    order_id: {
    ...
    issued_from: {
      ...
      enum: {
        ...
        message: `Invalid Type -> {{VALUE}} , it must be one of the ${(item_issued_from?.pressing_factory, item_issued_from?.polishing_factory, item_issued_from?.color_factory, item_issued_from?.polishing_factory, item_issued_from?.polishing_factory)}`,
      },
...
indexed_fields?.forEach((field) => issue_for_polishing_schema.index(...field));

export const issue_for_polishing_model = mongoose.model(
  'issued_for_polishing_details',
  issue_for_polishing_schema,
  'issued_for_polishing_details'
);

const issue_for_polishing_view_schema = new mongoose.Schema({}, { autoCreate: false, autoIndex: false, strict: false });
export const issue_for_polishing_view_model = mongoose.model('issue_for_polishing_details_view', issue_for_polishing_view_schema, 'issue_for_polishing_details_view');

(async function () {
  await issue_for_polishing_view_model.createCollection({
    viewOn: "issued_for_polishing_details", pipeline: [
      {
        $sort: {
          updatedAt: -1,
          _id: -1
