...
raw_machine_data_schema.index({ DressingDate: 1, ItemSrNo: 1 }, { unique: true });
...
raw_machine_data_schema.pre('save', async function (next) {
    ...
    const max_sr_no = await mongoose.model('dressing_machine_raw_data').findOne({ DressingDate: this.DressingDate }).sort({ ItemSrNo: -1 }).select("ItemSrNo");

    this.ItemSrNo = max_sr_no ? max_sr_no?.ItemSrNo + 1 : 1
    next()
})

const dressing_raw_machine_data_model = mongoose.model('dressing_machine_raw_data', raw_machine_data_schema, 'dressing_machine_raw_data');
