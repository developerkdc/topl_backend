...
const issue_for_canvas_schema = new mongoose.Schema(
  {
    ...
    issued_from: {
      ...
      enum: {
        ...
        message: `Invalid Type -> {{VALUE}} , it must be one of the ${(item_issued_from?.pressing_factory, item_issued_from?.bunito_factory, item_issued_from?.color_factory, item_issued_from?.canvas_factory, item_issued_from?.polishing_factory)}`,
      },
...
const issue_for_canvas_view_schema = new mongoose.Schema(
  {},
  { autoCreate: false, autoIndex: false, strict: false }
);
export const issue_for_canvas_view_model = mongoose.model(
  'issue_for_canvas_details_view',
  issue_for_canvas_view_schema,
  'issue_for_canvas_details_view'
);

(async function () {
  await issue_for_canvas_view_model.createCollection({
    viewOn: 'issued_for_canvas_details',
    pipeline: [
      {
        $sort: {
          updatedAt: -1,
