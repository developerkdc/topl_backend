...
const issue_for_canvas_schema = new mongoose.Schema(
  {
    ...
    issued_from: {
      ...
      enum: {
        ...
        message: `Invalid Type -> {{VALUE}} , it must be one of the ${(item_issued_from?.pressing_factory, item_issued_from?.canvas_factory, item_issued_from?.color_factory, item_issued_from?.canvas_factory, item_issued_from?.polishing_factory)}`,
      },
...
export const issue_for_canvas_model = mongoose.model(
  'issued_for_canvas_details',
  issue_for_canvas_schema,
  'issued_for_canvas_details'
);
// export default issue_for_canvas_model;

const issue_for_canvas_view_schema = new mongoose.Schema({}, { autoCreate: false, autoIndex: false, strict: false });
export const issue_for_canvas_view_model = mongoose.model('issue_for_canvas_details_view', issue_for_canvas_view_schema, 'issue_for_canvas_details_view');

(async function () {
  await issue_for_canvas_view_model.createCollection({
    viewOn: "issued_for_canvas_details", pipeline: [
      {
        $sort: {
          updatedAt: -1,
          _id: -1
