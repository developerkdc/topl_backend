...
const canvas_history_schema = new mongoose.Schema(
    {
        sr_no: Number,
        canvas_item_id: {
...
const indexed_fields = [[{ issue_status: 1 }], [{ sr_no: 1 }, { unique: true }], [{ canvas_item_id: 1 }]]
indexed_fields?.forEach((field) => canvas_history_schema.index(...field))
const canvas_history_model = mongoose.model(
    'canvas_history_details',
    canvas_history_schema,
    'canvas_history_details'
);

export default canvas_history_model;
