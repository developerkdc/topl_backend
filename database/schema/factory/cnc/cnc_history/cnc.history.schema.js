...
const cnc_history_schema = new mongoose.Schema(
  {
    ...
    issue_status: {
      ...
      enum: {
        ...
        message: `Invalid Type -> {{VALUE}} , it must be one of the ${[item_issued_from?.pressing_factory, item_issued_from?.cnc_factory, item_issued_from?.color_factory, item_issued_from?.bunito_factory, item_issued_from?.polishing_factory, item_issued_from?.packing]?.join(', ')}`,
      },
