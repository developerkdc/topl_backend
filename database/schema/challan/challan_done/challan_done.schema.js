...
const cnc_done_schema = new mongoose.Schema({
    ...
    raw_material: {
        ...
        enum: {
            ...
            message: 'Invalid Issued from value -> {{VALUE}}. It must be one of the : ' +
...
const indexed_fields = [[{ irn_number: 1 }, { unique: true }], [{ acknowledgement_number: 1 }, { unique: true }], [{ eway_bill_no: 1 }, { unique: true }], [{ challan_no: 1 }], [{ transport_document_no: 1 }], [{ raw_material: 1 }]];

indexed_fields.forEach((index) => cnc_done_schema.index(...index));
const challan_done_model = mongoose.model('challan_done_details', cnc_done_schema, 'challan_done_details');
