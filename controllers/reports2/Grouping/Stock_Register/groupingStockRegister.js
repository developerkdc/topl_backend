...
export const GroupingStockRegisterExcel = catchAsync(
  async (req, res, next) => {
    ...
    const pipeline = [
      ...
      // Stage 4 – compute per-item issue totals from history (sheets + SQM)
      {
        $addFields: {
          ...
          // Sum amount and expense_amount from history records (not from item document)
    ...
    const orphanPipeline = [
      ...
      },
      {
        $group: {
          ...
          grouping_done_date: { $first: '$session.grouping_done_date' },
          log_no_code: { $first: '$item.log_no_code' },
    ...
    const dateWiseKey = (r) =>
      `${r._id.item_sub_category_name}|${r._id.item_name}|${r._id.grouping_done_date?.toISOString?.() ?? r._id.grouping_done_date}|${r._id.log_no_code}|${r._id.thickness}`;

    for (const r of rawRows) {
      rowMap.set(dateWiseKey(r), {
        _id: r._id,
    ...
    const mergedRows = Array.from(rowMap.values()).sort((a, b) => {
      ...
      const da = a._id.grouping_done_date?.getTime?.() ?? 0;
      const db = b._id.grouping_done_date?.getTime?.() ?? 0;
      if (da !== db) return da - db;
      const la = a._id.log_no_code ?? '';
      const lb = b._id.log_no_code ?? '';
