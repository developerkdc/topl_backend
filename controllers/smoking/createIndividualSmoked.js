...
//     console.log(issuedForSmokingIndividualItemsExist, "sfafg");
...
//       for (const itemId of issuedForSmokingIndividualItemsExist) {
...
//     const smokingIndividual = issuedForSmokingIndividualItemsExist.map((e) => ({
//       // issued_individual_smoking_id: e._id,
//       item_details: e.item_id,
//       issued_smoking_quantity: e.issued_smoking_quantity,
...
//     // Insert Smoking Individual items into the IndividualSmokeModel
//     const saveData = await IndividualSmokeModel.insertMany(smokingIndividual, {
//       session,
//     });

//     // if create Smoking Individual items doen delete item from issued Individual smoke model
//     // if (saveData) {
//     //   await IssuedForSmokingIndividualModel.deleteMany({
//     //     _id: { $in: item_details },
//     //   }).session(session);
//     // }
//     if (saveData) {
//       await IssuedForSmokingIndividualModel.updateMany(
//         { _id: { $in: item_details } },
//         { $set: { status: "smoked" } }
//       ).session(session);
//     }
...
export const CreateIndividualSmoked = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    const smokingIndividual = issuedForSmokingIndividualItemsExist?.map(
      (e) => ({
        // issued_individual_smoking_id: e._id,
        item_details: e.item_id,
        issued_smoking_quantity: e.issued_smoking_quantity,
