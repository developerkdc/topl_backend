...
export const CreateGroupSmoked = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    const issuedForSmokingGroupItemsExist = await IssuedForSmokingGroupModel.find({
      _id: { $in: extracted_group_details },
    })
      .populate("group_id")
      .session(session);
      
    if (issuedForSmokingGroupItemsExist.length != extracted_group_details.length) {
    ...
    // }
    if (saveData) {
      await IssuedForSmokingGroupModel.updateMany({ _id: { $in: extracted_group_details } }, { $set: { status: "smoked" } }).session(
        session
      );
    }
