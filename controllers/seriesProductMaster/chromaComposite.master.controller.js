...
export const updateChromaCompositeDetails = catchAsync(async (req, res, next) => {
    const { id } = req.params;
    const reqBody = req.body;
    const authUserDetails = req.userDetails;

    if (!id) {
        return next(new ApiError("ChromaComposite id is missing", StatusCodes.NOT_FOUND))
    }
    ...
    const updateResponse = await chromaCompositeModel.updateOne({_id:id}, {
        $set: updatedDetails,
...
export const fetchChromaCompositeList = catchAsync(async (req, res, next) => {
    ...
    const chromaCompositeData = await chromaCompositeModel.aggregate(listAggregate);
