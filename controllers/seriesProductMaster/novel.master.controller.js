...
import novelModel from '../../database/schema/seriesProductMaster/novel.master.schema.js';
...
export const addNovel = catchAsync(async (req, res, next) => {
  ...
  const newData = new novelModel(itemDetails);
...
export const updateNovelDetails = catchAsync(async (req, res, next) => {
  const { id } = req.params;
  const reqBody = req.body;
  const authUserDetails = req.userDetails;
  ...
  const updateResponse = await novelModel.updateOne(
    { _id: id },
    {
      $set: updatedDetails,
    },
...
export const fetchNovelList = catchAsync(async (req, res, next) => {
  const {
    page = 1,
    limit = 10,
    sortBy = 'createdAt',
  ...
  const result = await novelModel.aggregate(listAggregate);
  ...
  const totalDocument = await novelModel.aggregate(totalAggregate);
  ...
  const response = new ApiResponse(
    StatusCodes.OK,
    'Novel Details Fetched Successfully',
    {
      data: result,
...
export const fetchSingleNovel = catchAsync(async (req, res, next) => {
  const { id } = req.params;
  ...
  const result = await novelModel.aggregate(aggregate);
...
export const dropdownNovel = catchAsync(async (req, res, next) => {
  const result = await novelModel.aggregate([
    {
      $match: { status: true },
    },
    {
  ...
  const response = new ApiResponse(
    StatusCodes.OK,
    'Novel Dropdown Fetched Successfully',
    result
  );
...
export const updateStatus = catchAsync(async (req, res, next) => {
  ...
  const update_result = await novelModel.findByIdAndUpdate(id, {
    $set: {
      status: status,
    },
  });
