...
export const addItems = catchAsync(async (req, res) => {
    ...
    if (!name) {
        return res.json(new ApiResponse(StatusCodes.INTERNAL_SERVER_ERROR, "Subcategory name is missing"))
    };
    const checkIfAlreadyExists = await itemSubCategoryModel.find({ name: name });
    if (checkIfAlreadyExists.length > 0) {
        return res.json(new ApiResponse(StatusCodes.OK, "SubCategory already exists"))
    };
    ...
    const newItemCatgory = new itemSubCategoryModel({
        sr_no: newMax, name, created_by
    });
    await newItemCatgory.save();

    return res.json(new ApiResponse(StatusCodes.OK, "Item subcategory created successfully", newItemCatgory))
});
...
export const editItemSubCategory = catchAsync(async (req, res) => {
    ...
    if (!validateSubCategory) {
        return res.json(new ApiResponse(StatusCodes.INTERNAL_SERVER_ERROR, "Invalid SubCategory id"))
    };

    const updatedData = await itemSubCategoryModel.findByIdAndUpdate(id, { $set: req.body }, { runValidators: true, new: true });
    if (!updatedData) {
        return res.json(new ApiResponse(StatusCodes.INTERNAL_SERVER_ERROR, "Err updating subcategory"))
    };
    return res.json(new ApiResponse(StatusCodes.OK, "subcategory updated successfully", updatedData))
});
...
export const listItemSubCategories = catchAsync(async (req, res) => {
    ...
    const totalDocs = await itemSubCategoryModel.countDocuments({ ...searchQuery })
