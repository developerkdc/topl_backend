...
export const addMachine = catchAsync(async (req, res) => {
    ...
    const checkIfAlreadyExists = await machineModel.find({ machine_name: machine_name });
    ...
    const newMachine = new machineModel({
        sr_no: newMax, machine_name, department, created_by
    });
    await newMachine.save();

    return res.json(new ApiResponse(StatusCodes.OK, "Machine created successfully", newDept))
});
...
export const editMachineDetails = catchAsync(async (req, res) => {
    ...
    if (!validateMachine) {
        return res.json(new ApiResponse(StatusCodes.INTERNAL_SERVER_ERROR, "Invalid machine id"))
    };

    const updatedData = await machineModel.findByIdAndUpdate(id, { $set: req.body }, { runValidators: true, new: true });
    if (!updatedData) {
        return res.json(new ApiResponse(StatusCodes.INTERNAL_SERVER_ERROR, "Err updating machine"))
    };
