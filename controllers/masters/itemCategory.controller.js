...
export const addItems = catchAsync(async (req, res) => {
    ...
    if (!category || !product_hsn_code || !calculate_unit) {
        return res.json(new ApiResponse(StatusCodes.INTERNAL_SERVER_ERROR, "All fields are required"))
    };
    const checkIfAlreadyExists = await itemCategoryModel.find({ category: category });
    ...
    const newItemCatgory = new itemCategoryModel({
        sr_no: newMax, category, product_hsn_code, calculate_unit, created_by
    });
    await newItemCatgory.save();

    return res.json(new ApiResponse(StatusCodes.OK, "Item category created successfully", newItemCatgory))
});
...
export const editItemCatgory = catchAsync(async (req, res) => {
    ...
    const updatedData = await itemCategoryModel.findByIdAndUpdate(id, { $set: req.body }, { runValidators: true, new: true });
    if (!updatedData) {
        return res.json(new ApiResponse(StatusCodes.INTERNAL_SERVER_ERROR, "Err updating category"))
    };
    return res.json(new ApiResponse(StatusCodes.OK, "category updated successfully"))
});
...
export const fetchAllCategories = catchAsync(async (req, res) => {
    ...
    return res.json(new ApiResponse(StatusCodes.OK, "All categories fetched successfully..", allData))
})