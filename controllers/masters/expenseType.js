...
export const UpdateExpenseTypeMaster = catchAsync(async (req, res) => {
  ...
  if (!mongoose.Types.ObjectId.isValid(userId)) {
    return res.status(400).json({ result: [], status: false, message: "Invalid ExpenseType ID" });
  }
  const user = await ExpenseTypeModel.findByIdAndUpdate(userId, { $set: updateData }, { new: true, runValidators: true });
  if (!user) {
