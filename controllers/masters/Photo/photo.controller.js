...
export const updatePhoto = catchAsync(async (req, res, next) => {
    ...
    try {
        ...
        try {
            ...
            if (removeFile) {
                ...
                if (!Array.isArray(removeImages)) {
                    throw new Error("Invalid data type: removeImages field should be array")
                }
            }
        } catch (error) {
            console.log(error)
        ...
        const updatePhotoData = await photoModel.updateOne({ _id: id }, {
            $set: photoData,
            $push: {
        ...
        if (removeImages && removeImages.length > 0) {
            const removePhoto = await photoModel.updateOne({ _id: id }, {
                $pull: {
