...
export const addSeries = catchAsync(async (req, res) => {
    ...
    if (!series_name) {
        return res.json(new ApiResponse(StatusCodes.INTERNAL_SERVER_ERROR, "series name is required"))
    };
    const checkIfAlreadyExists = await seriesModel.find({ series_name: series_name });
...
export const editSeries = catchAsync(async (req, res) => {
    ...
    if (!validateDept) {
        return res.json(new ApiResponse(StatusCodes.INTERNAL_SERVER_ERROR, "Invalid series id"))
    };

    const updatedData = await seriesModel.findByIdAndUpdate(id, { $set: req.body }, { runValidators: true, new: true });
    if (!updatedData) {
        return res.json(new ApiResponse(StatusCodes.INTERNAL_SERVER_ERROR, "Err updating series"))
    };
    return res.json(new ApiResponse(StatusCodes.OK, "Series updated successfully"))
});
