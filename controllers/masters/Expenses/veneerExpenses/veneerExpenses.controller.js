...
export const veneerExpenses_invoice_listing = catchAsync(async function (req, res, next) {
  ...
  const List_veneer_invoice_details = await veneer_inventory_invoice_model.aggregate(aggregate_stage);
...
export const veneerExpenses_item_listing_by_invoice = catchAsync(async (req, res, next) => {
  ...
  const veneerExpense_item_by_invoice = await veneer_inventory_items_model.aggregate(aggregate_stage);
  const veneerExpense_invoice = await veneer_inventory_invoice_model.findOne({ _id: invoice_id });
...
export const add_veneerExpenses = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    const updateVeneerItemsExpenses = await veneer_inventory_items_model.aggregate([
      ...
      {
        $merge: {
          into: "veneer_inventory_items_model",
          whenMatched: "merge",
        },
      },
    ]);
