...
export const create_challan = catchAsync(async (req, res) => {
    ...
    if (!challan_details) {
        throw new ApiError("Challan details are required.", StatusCodes.BAD_REQUEST)
    }
...
export const edit_challan_details = catchAsync(async (req, res) => {
    ...
    try {
        ...
        const challan_done_details = await challan_done_model.findById(id).session(session);
        if (!challan_done_details) {
            throw new ApiError("Challan done details not found", StatusCodes.NOT_FOUND)
        };

        const update_existing_issue_for_challan_status_result = await issue_for_challan_model.updateMany({ _id: { $in: challan_done_details?.raw_material_items } }, {
            $set: {
        ...
        if (update_existing_issue_for_challan_status_result?.matchedCount === 0) {
            throw new ApiError("Failed to update issue for challan status", StatusCodes.BAD_REQUEST)
        }
        if (!update_existing_issue_for_challan_status_result?.acknowledged || update_existing_issue_for_challan_status_result?.modifiedCount === 0) {
            throw new ApiError("Failed to update issue for challan status", StatusCodes.BAD_REQUEST)
        };
        const updated_details = {
            ...challan_details,
        ...
        const update_challan_details = await challan_done_model.updateOne({ _id: challan_done_details?._id }, { $set: updated_details }, { session });


        if (!update_challan_details?.acknowledged || update_challan_details?.modifiedCount === 0) {
            throw new ApiError('Failed to update challan details.', StatusCodes.BAD_REQUEST);
        }
