...
class RevertIssueForChallan {
    constructor(id, userDetails, session) {
        if (!isValidObjectId(id)) {
            throw new ApiError('Invalid Issue for challan ID', StatusCodes.BAD_REQUEST);
        }
        this.issued_challan_data = null;
        this.id = id;
    ...
    async fetch_issue_for_challan_data() {
        ...
        this.issued_challan_data = issued_challan_data;
        return issued_challan_data;
    }

    async update_inventory_item_status() {
        try {
            await this.fetch_issue_for_challan_data();

            const issued_from = this.issued_challan_data?.issued_from;
    ...
    async LOG() {
        const update_log_item = await log_inventory_items_model?.findOneAndUpdate(
            { _id: this.issued_challan_data?.issued_item_details?._id },
            {
                $set: {
                    issue_status: null,
        ...
        const update_plywood_item =
            await plywood_inventory_items_details.findOneAndUpdate(
                { _id: this.issued_challan_data?.issued_item_details?._id },
                {
                    $inc: {
                        available_sheets:
                            this.issued_challan_data?.issued_item_details?.issued_sheets || 0,
                        available_amount:
                            this.issued_challan_data?.issued_item_details?.issued_amount || 0,
                        available_sqm:
                            this.issued_challan_data?.issued_item_details?.issued_sqm || 0,
                    },
                },
                { session: this.session }
            );
        ...
        const update_mdf_item = await mdf_inventory_items_details.findOneAndUpdate(
            { _id: this.issued_challan_data?.issued_item_details?._id },
            {
                $inc: {
                    ...
                    available_amount:
                        this.issued_challan_data?.issued_item_details?.issued_amount || 0,
                    available_sqm: this.issued_challan_data?.issued_item_details?.issued_sqm || 0,
                },
            },
            { session: this.session, new: true }
        );
                ...
                { _id: this.issued_challan_data?.issued_item_details?._id },
                {
                    $inc: {
                        available_number_of_roll:
                            this.issued_challan_data?.issued_item_details?.issued_number_of_roll || 0,
                        available_amount:
                            this.issued_challan_data?.issued_item_details?.issued_amount || 0,
                        available_sqm:
                            this.issued_challan_data?.issued_item_details?.issued_sqm || 0,
        ...
        const update_other_goods_item =
            await othergoods_inventory_items_details.findOneAndUpdate(
                { _id: this.issued_challan_data?.issued_item_details?._id },
                {
                    $inc: {
                        available_quantity:
                            this.issued_challan_data?.issued_item_details?.issued_quantity || 0,
                        available_amount:
                            this.issued_challan_data?.issued_item_details?.issued_amount || 0,
                    },
                },
                ...
                { _id: this.issued_challan_data?.issued_item_details?._id },
                {
                    $inc: {
                        ...
                        'available_details.sqm':
                            this.issued_challan_data?.issued_item_details?.issued_sqm,
                    },
                    $set: {
                        updated_by: this.userDetails?._id,
                    },
...
export default RevertIssueForChallan;
