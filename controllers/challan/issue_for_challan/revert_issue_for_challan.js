...
class RevertIssueForChallan {
  ...
  async fetch_issue_for_challan_data() {
    const issued_challan_data = await issue_for_challan_model
      .findById(this.id)
      .lean();
    if (!issued_challan_data) {
      throw new ApiError(
        'Issue for challan data not found',
        StatusCodes.BAD_REQUEST
      );
    }
        ...
        { _id: this.issued_challan_data?.issued_item_details?._id },
        {
          $inc: {
            available_number_of_roll:
              this.issued_challan_data?.issued_item_details
                ?.issued_number_of_roll || 0,
            available_amount:
              this.issued_challan_data?.issued_item_details?.issued_amount || 0,
            available_sqm:
              this.issued_challan_data?.issued_item_details?.issued_sqm || 0,
    ...
    const update_other_goods_item =
      await othergoods_inventory_items_details.findOneAndUpdate(
        { _id: this.issued_challan_data?.issued_item_details?._id },
        {
          $inc: {
            available_quantity:
              this.issued_challan_data?.issued_item_details?.issued_quantity ||
              0,
            available_amount:
              this.issued_challan_data?.issued_item_details?.issued_amount || 0,
          },
