...
export const fetch_all_items_by_raw_material = catchAsync(async (req, res) => {
  ...
  const issue_for_challan_items =
    await issue_for_challan_model.aggregate(pipeline);
  const response = new ApiResponse(
    StatusCodes.OK,
    'Issue for challan items fetched successfully',
    issue_for_challan_items
  );
  return res.status(StatusCodes.OK).json(response);
});
