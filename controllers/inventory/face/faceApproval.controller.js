...
export const faceApproval_invoice_listing = catchAsync(async function (req, res, next) {
  ...
  const List_face_invoice_details = await face_approval_inventory_invoice_model.aggregate(aggregate_stage);

  const totalCount = await face_approval_inventory_invoice_model.countDocuments({
    ...match_query,
  });
...
export const faceApproval_item_listing_by_invoice = catchAsync(async (req, res, next) => {
  ...
  const faceExpense_invoice_data = await face_approval_inventory_invoice_model.aggregate([
    {
      $match: {
        _id: new mongoose.Types.ObjectId(document_id),
        invoice_id: new mongoose.Types.ObjectId(invoice_id),
...
export const face_approve_invoice_details = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    const update_face_invoice = await face_inventory_invoice_details.updateOne(
      { _id: invoice_id },
      {
        $set: {
          ...invoiceData,
    ...
    if (!update_face_invoice.acknowledged || update_face_invoice.modifiedCount <= 0)
