...
export const listing_face_inventory = catchAsync(async (req, res, next) => {
  const optimizedResult = await runInventoryListingPagination({
    itemsModel: face_inventory_items_details,
    invoiceModel: face_inventory_invoice_details,
    invoiceAlias: 'face_invoice_details',
    invoicePrefixes: [{ prefix: 'face_invoice_details.' }],
    staticMatch: {
    ...
    try {
      ...
      if (!sendForApproval) {
        ...
        const add_approval_item_details =
          await face_approval_inventory_items_model.insertMany(updated_items, {
            session,
          });
...
export const fetch_face_history = catchAsync(async (req, res, next) => {
  const optimizedResult = await runInventoryHistoryPagination({
    historyModel: face_history_model,
    itemViewModel: face_inventory_items_view_modal,
    itemLocalField: 'face_item_id',
    itemAlias: 'face_item_details',
    itemLookupPipeline: [
      {
        $project: {
          created_user: 0,
