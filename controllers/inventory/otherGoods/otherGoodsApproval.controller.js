...
export const otherGoodsApproval_invoice_listing = catchAsync(
  async function (req, res, next) {
    ...
    const List_otherGoods_invoice_details =
      await otherGoods_approval_inventory_invoice_model.aggregate(
        aggregate_stage
      );

    const totalCount =
      await otherGoods_approval_inventory_invoice_model.countDocuments({
        ...match_query,
      });
...
export const otherGoodsApproval_item_listing_by_invoice = catchAsync(
  async (req, res, next) => {
    ...
    const approval_invoice_details =
      await otherGoods_approval_inventory_invoice_model
        .findOne({
    ...
    const otherGoodsExpense_invoice_data =
      await otherGoods_approval_inventory_invoice_model.aggregate([
        {
          $match: {
            _id: new mongoose.Types.ObjectId(document_id),
    ...
    const otherGoodsExpense_item_by_invoice =
      await otherGoods_approval_inventory_items_model.aggregate(
        aggregate_stage
      );
...
export const otherGoods_approve_invoice_details = catchAsync(
  async (req, res, next) => {
    ...
    try {
      ...
      const invoice_details = await otherGoods_approval_inventory_invoice_model
        .findOneAndUpdate(
          {
            _id: document_id,
      ...
      const update_otherGoods_invoice =
      ...
      if (
        !update_otherGoods_invoice.acknowledged ||
        update_otherGoods_invoice.modifiedCount <= 0
      )
        return next(new ApiError('Failed to approve invoice'), 400);

      const items_details = await otherGoods_approval_inventory_items_model
        .find({
...
export const otherGoods_reject_invoice_details = catchAsync(
  async (req, res, next) => {
    const session = await mongoose.startSession();
    session.startTransaction();
    try {
