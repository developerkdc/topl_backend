...
export const fleeceApproval_invoice_listing = catchAsync(async function (req, res, next) {
  ...
  const List_fleece_invoice_details = await fleece_approval_inventory_invoice_model.aggregate(aggregate_stage);
...
export const fleeceApproval_item_listing_by_invoice = catchAsync(async (req, res, next) => {
  const invoice_id = req.params.invoice_id;
  const document_id = req.params._id;

  const approval_invoice_details = await fleece_approval_inventory_invoice_model
    .findOne({
  ...
  const fleeceExpense_invoice_data = await fleece_approval_inventory_invoice_model.aggregate([
    {
      $match: {
        _id: new mongoose.Types.ObjectId(document_id),
        invoice_id: new mongoose.Types.ObjectId(invoice_id),
  ...
  const fleeceExpense_invoice = fleeceExpense_invoice_data?.[0];
  ...
  const aggregate_stage = [
    ...
    ...(isApprovalPending
      ? [
          {
            $lookup: {
              from: "fleece_inventory_items_details",
...
export const fleece_approve_invoice_details = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    const invoice_details = await fleece_approval_inventory_invoice_model
      .findOneAndUpdate(
        {
          _id: document_id,
    ...
    const update_fleece_invoice = await fleece_inventory_invoice_modal.updateOne(
      { _id: invoice_id },
      {
        $set: {
          ...invoiceData,
    ...
    if (!update_fleece_invoice.acknowledged || update_fleece_invoice.modifiedCount <= 0)
      return next(new ApiError("Failed to approve invoice"), 400);

    const items_details = await fleece_approval_inventory_items_model
      .find({
