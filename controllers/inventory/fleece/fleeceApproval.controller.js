...
export const fleeceApproval_invoice_listing = catchAsync(
  async function (req, res, next) {
    ...
    const totalCount =
      await fleece_approval_inventory_invoice_model.countDocuments({
        ...match_query,
      });
...
export const fleeceApproval_item_listing_by_invoice = catchAsync(
  async (req, res, next) => {
    ...
    const approval_invoice_details =
      await fleece_approval_inventory_invoice_model
        .findOne({
    ...
    const fleeceExpense_invoice_data =
      await fleece_approval_inventory_invoice_model.aggregate([
        {
          $match: {
            _id: new mongoose.Types.ObjectId(document_id),
            invoice_id: new mongoose.Types.ObjectId(invoice_id),
    ...
    const fleeceExpense_item_by_invoice =
      await fleece_approval_inventory_items_model.aggregate(aggregate_stage);
...
export const fleece_approve_invoice_details = catchAsync(
  async (req, res, next) => {
    ...
    try {
      ...
      if (
        !update_fleece_invoice.acknowledged ||
        update_fleece_invoice.modifiedCount <= 0
      )
      ...
      const approval_invoice_items_data =
        await fleece_approval_inventory_items_model.aggregate([
          {
            $match: {
              approval_invoice_id: invoice_details?._id,
              invoice_id: new mongoose.Types.ObjectId(invoice_id),
      ...
      await fleece_inventory_items_modal.insertMany(
...
export const fleece_reject_invoice_details = catchAsync(
  async (req, res, next) => {
    const session = await mongoose.startSession();
    session.startTransaction();
    try {
