...
export const listing_fleece_inventory = catchAsync(async (req, res, next) => {
  const optimizedResult = await runInventoryListingPagination({
    itemsModel: fleece_inventory_items_modal,
    invoiceModel: fleece_inventory_invoice_modal,
    invoiceAlias: 'fleece_invoice_details',
    invoicePrefixes: [{ prefix: 'fleece_invoice_details.' }],
    staticMatch: {
      available_sqm: { $ne: 0 },
    },
    req,
...
export const fetch_fleece_history = catchAsync(async (req, res, next) => {
  const optimizedResult = await runInventoryHistoryPagination({
    historyModel: fleece_history_model,
    itemViewModel: fleece_inventory_items_view_modal,
    itemLocalField: 'fleece_item_id',
    itemAlias: 'fleece_item_details',
    itemLookupPipeline: [
      {
        $project: {
          created_user: 0,
