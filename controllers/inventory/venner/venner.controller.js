...
export const edit_veneer_item_invoice_inventory = catchAsync(async (req, res, next) => {
  const session = await mongoose.startSession();
  session.startTransaction();
  try {
...
export const edit_veneer_invoice_inventory = catchAsync(async (req, res, next) => {
  const invoice_id = req.params?.invoice_id;
...
export const inward_sr_no_dropdown = catchAsync(async (req, res, next) => {
  const item_sr_no = await veneer_inventory_invoice_model.distinct("inward_sr_no");
  return res.status(200).json({
    statusCode: 200,
    status: "success",
    data: item_sr_no,
...
export const veneer_item_listing_by_invoice = catchAsync(async (req, res, next) => {
  ...
  const single_invoice_list_veneer_inventory_details = await veneer_inventory_items_view_modal.aggregate(aggregate_stage);
