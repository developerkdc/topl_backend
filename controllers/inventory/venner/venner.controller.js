...
import {
  ...
  venner_inventory_items_view_modal,
...
export const listing_venner_inventory = catchAsync(async (req, res, next) => {
  const {
    page = 1,
    limit = 10,
    sortBy = "updatedAt",
  ...
  const List_venner_inventory_details =
    await venner_inventory_items_view_modal.aggregate(aggregate_stage);
...
export const add_venner_inventory = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    const inward_sr_no = await venner_inventory_invoice_details.aggregate([
      {
        $group: {
          _id: null,
    ...
    const add_invoice_details = await venner_inventory_invoice_details.create(
      [
        {
    ...
    const add_items_details = await venner_inventory_items_details.insertMany(
      items_details,
      {
        session,
      }
...
export const add_single_venner_item_inventory = catchAsync(
  async (req, res, next) => {
    ...
    const add_item_details = await venner_inventory_items_details.create({
      ...item_details,
    });
...
export const edit_venner_item_inventory = catchAsync(async (req, res, next) => {
  const item_id = req.params?.item_id;
  const item_details = req.body?.item_details;

  const update_item_details = await venner_inventory_items_details.updateOne(
    { _id: item_id },
    {
      $set: {
        ...item_details,
...
export const edit_venner_invoice_inventory = catchAsync(
  async (req, res, next) => {
    const invoice_id = req.params?.invoice_id;
    const invoice_details = req.body?.invoice_details;

    const update_voice_details =
      await venner_inventory_invoice_details.updateOne(
        { _id: invoice_id },
        {
          $set: invoice_details,
        }
