...
export const flitchApproval_invoice_listing = catchAsync(
  async function (req, res, next) {
    ...
    const totalCount =
      await flitch_approval_inventory_invoice_model.countDocuments({
        ...match_query,
      });
...
export const flitchApproval_item_listing_by_invoice = catchAsync(
  async (req, res, next) => {
    ...
    const approval_invoice_details =
      await flitch_approval_inventory_invoice_model
        .findOne({
    ...
    const flitchExpense_invoice_data =
      await flitch_approval_inventory_invoice_model.aggregate([
        {
          $match: {
            _id: new mongoose.Types.ObjectId(document_id),
            invoice_id: new mongoose.Types.ObjectId(invoice_id),
    ...
    const flitchExpense_invoice = flitchExpense_invoice_data?.[0];
    ...
    const aggregate_stage = [
      ...
      ...(isApprovalPending
        ? [
            {
              $lookup: {
                from: 'flitch_inventory_items_details',
                localField: 'flitch_item_id',
                foreignField: '_id',
                as: 'previous_data',
              },
            },
    ...
    const flitchExpense_item_by_invoice =
      await flitch_approval_inventory_items_model.aggregate(aggregate_stage);
...
export const flitch_approve_invoice_details = catchAsync(
  async (req, res, next) => {
    ...
    try {
      ...
      const invoice_details = await flitch_approval_inventory_invoice_model
        .findOneAndUpdate(
          {
            _id: document_id,
      ...
      if (
        !update_flitch_invoice.acknowledged ||
        update_flitch_invoice.modifiedCount <= 0
      )
        return next(new ApiError('Failed to approve invoice'), 400);

      const items_details = await flitch_approval_inventory_items_model
        .find({
      ...
      await flitch_inventory_items_model.insertMany(
...
export const flitch_reject_invoice_details = catchAsync(
  async (req, res, next) => {
    const session = await mongoose.startSession();
    session.startTransaction();
    try {
