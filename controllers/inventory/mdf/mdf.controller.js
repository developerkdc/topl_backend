...
export const add_mdf_inventory = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    const add_items_details = await mdf_inventory_items_details.insertMany(items_details, {
      session,
    });
...
export const add_single_mdf_item_inventory = catchAsync(async (req, res, next) => {
  ...
  return res.status(201).json(new ApiResponse(StatusCodes.CREATED, "Inventory has added successfully", add_item_details));
});
...
export const mdfLogsCsv = catchAsync(async (req, res) => {
  ...
  return res.json(new ApiResponse(StatusCodes.OK, "Csv downloaded successfully...", excelLink));
});

export const edit_mdf_item_invoice_inventory = catchAsync(async (req, res, next) => {
  const session = await mongoose.startSession();
  session.startTransaction();
  try {
    const invoice_id = req.params?.invoice_id;
    ...
    const update_invoice_details = await mdf_inventory_invoice_details.updateOne(
      { _id: invoice_id },
      {
        $set: {
          ...invoice_details,
    ...
    const all_invoice_items = await mdf_inventory_items_details.deleteMany({ invoice_id: invoice_id }, { session });
...
export const mdf_item_listing_by_invoice = catchAsync(async (req, res, next) => {
  const invoice_id = req.params.invoice_id;
  ...
  const single_invoice_list_mdf_inventory_details = await mdf_inventory_items_view_modal.aggregate(aggregate_stage);
...
export const inward_sr_no_dropdown = catchAsync(async (req, res, next) => {
  const item_sr_no = await mdf_inventory_invoice_details.distinct("inward_sr_no");
  return res.status(200).json({
    statusCode: 200,
    status: "success",
    data: item_sr_no,
