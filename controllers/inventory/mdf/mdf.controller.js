...
// MDF Stock Report with Opening, Receive, Consume, Sales, Closing
export const mdfStockReportCsv = catchAsync(async (req, res, next) => {
  const { startDate, endDate } = req.body;
  const filter = req.body?.filter || {};

  console.log('MDF Stock Report Request - Start Date:', startDate);
  console.log('MDF Stock Report Request - End Date:', endDate);
  console.log('MDF Stock Report Request - Filter:', filter);
  ...
  try {
    // Step 1: Get all unique MDF items with their current available stock
    ...
    const stockData = await Promise.all(
      currentInventory.map(async (item) => {
        ...
        // Calculate consumption (issues for order, pressing)
        ...
        // Get issue pressing from the result
    ...
    // Generate Excel file
    const excelLink = await createMdfStockReportExcel(
      activeStockData,
      startDate,
      endDate,
      filter
