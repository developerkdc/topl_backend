...
export const mdfApproval_invoice_listing = catchAsync(
  async function (req, res, next) {
    ...
    const totalCount =
      await mdf_approval_inventory_invoice_model.countDocuments({
        ...match_query,
      });
...
export const mdfApproval_item_listing_by_invoice = catchAsync(
  async (req, res, next) => {
    ...
    const approval_invoice_details = await mdf_approval_inventory_invoice_model
      .findOne({
    ...
    const mdfExpense_invoice_data =
      await mdf_approval_inventory_invoice_model.aggregate([
        {
          $match: {
            _id: new mongoose.Types.ObjectId(document_id),
            invoice_id: new mongoose.Types.ObjectId(invoice_id),
    ...
    const mdfExpense_invoice = mdfExpense_invoice_data?.[0];
    ...
    const mdfExpense_item_by_invoice =
      await mdf_approval_inventory_items_model.aggregate(aggregate_stage);
...
export const mdf_approve_invoice_details = catchAsync(
  async (req, res, next) => {
    ...
    try {
      ...
      const invoice_details = await mdf_approval_inventory_invoice_model
        .findOneAndUpdate(
          {
            _id: document_id,
      ...
      const update_mdf_invoice = await mdf_inventory_invoice_details.updateOne(
        { _id: invoice_id },
        {
          $set: {
            ...invoiceData,
      ...
      if (
        !update_mdf_invoice.acknowledged ||
        update_mdf_invoice.modifiedCount <= 0
      )
        return next(new ApiError('Failed to approve invoice'), 400);

      const items_details = await mdf_approval_inventory_items_model
        .find({
      ...
      const approval_invoice_items_data =
        await mdf_approval_inventory_items_model.aggregate([
          {
            $match: {
              approval_invoice_id: invoice_details?._id,
              invoice_id: new mongoose.Types.ObjectId(invoice_id),
      ...
      await mdf_inventory_items_details.insertMany(
