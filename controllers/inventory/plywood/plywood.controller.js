...
// Plywood Stock Report with Opening, Receive, Consume, Sales, Closing
export const plywoodStockReportCsv = catchAsync(async (req, res, next) => {
  ...
  try {
    // Step 1: Get all unique plywood items with their current available stock
    ...
    const stockData = await Promise.all(
      currentInventory.map(async (item) => {
        ...
        // Calculate consumption (issues for order, pressing, resizing)
        ...
        // Calculate issues for recalibration (resizing and pressing)
    ...
    // Generate Excel file
    const excelLink = await createPlywoodStockReportExcel(
      activeStockData,
      startDate,
      endDate,
      filter
