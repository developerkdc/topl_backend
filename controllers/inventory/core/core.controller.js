...
export const add_core_inventory = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    const latest_inward_sr_no =
      inward_sr_no?.length > 0 && inward_sr_no?.[0]?.latest_inward_sr_no ? inward_sr_no?.[0]?.latest_inward_sr_no + 1 : 1;
    ...
    const add_items_details = await core_inventory_items_details.insertMany(items_details, {
      session,
    });
...
export const add_single_core_item_inventory = catchAsync(async (req, res, next) => {
  ...
  return res.status(200).json(new ApiResponse(StatusCodes.OK, "Inventory Item has added successfully", add_item_details));
});
...
export const edit_core_item_inventory = catchAsync(async (req, res, next) => {
  ...
  return res.status(200).json(new ApiResponse(StatusCodes.OK, "Inventory Item has updated successfully", update_item_details));
});

export const edit_core_invoice_inventory = catchAsync(async (req, res, next) => {
  ...
  return res.status(200).json(new ApiResponse(StatusCodes.OK, "Inventory Invoice has updated successfully", update_voice_details));
});

export const core_item_listing_by_invoice = catchAsync(async (req, res, next) => {
  const invoice_id = req.params.invoice_id;
  ...
  const single_invoice_list_core_inventory_details = await core_inventory_items_view_modal.aggregate(aggregate_stage);
...
export const edit_core_item_invoice_inventory = catchAsync(async (req, res, next) => {
  const session = await mongoose.startSession();
  session.startTransaction();
  try {
    const invoice_id = req.params?.invoice_id;
    ...
    const update_invoice_details = await core_inventory_invoice_details.updateOne(
      { _id: invoice_id },
      {
        $set: {
          ...invoice_details,
    ...
    if (!update_invoice_details.acknowledged || update_invoice_details.modifiedCount <= 0)
      return next(new ApiError("Failed to update invoice", 400));

    const all_invoice_items = await core_inventory_items_details.deleteMany({ invoice_id: invoice_id }, { session });
    ...
    const update_item_details = await core_inventory_items_details.insertMany([...items_details], { session });

    await session.commitTransaction();
    session.endSession();
    return res.status(StatusCodes.OK).json(new ApiResponse(StatusCodes.OK, "Inventory item updated successfully", update_item_details));
  } catch (error) {
    console.log(error);
    await session.abortTransaction();
    session.endSession();
...
export const inward_sr_no_dropdown = catchAsync(async (req, res, next) => {
  const item_sr_no = await core_inventory_invoice_details.distinct("inward_sr_no");
  return res.status(200).json({
    statusCode: 200,
    status: "success",
    data: item_sr_no,
