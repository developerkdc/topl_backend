...
export const coreStockReportCsv = catchAsync(async (req, res, next) => {
  const { startDate, endDate, filter = {} } = req.body;

  console.log('Core Stock Report Request - Start Date:', startDate);
  console.log('Core Stock Report Request - End Date:', endDate);
  console.log('Core Stock Report Request - Filter:', filter);
  ...
  try {
    ...
    const stockData = await Promise.all(
      uniqueCombinations.map(async (combo) => {
        ...
        // Issued sqm (from core_history during period)
    ...
    const downloadLink = await createCoreStockReportExcel(
      activeStockData,
      startDate,
      endDate,
      filter
