...
export const coreApproval_invoice_listing = catchAsync(async function (req, res, next) {
  ...
  const List_core_invoice_details = await core_approval_inventory_invoice_model.aggregate(aggregate_stage);
...
export const coreApproval_item_listing_by_invoice = catchAsync(async (req, res, next) => {
  const invoice_id = req.params.invoice_id;
  const document_id = req.params._id;

  const approval_invoice_details = await core_approval_inventory_invoice_model
    .findOne({
  ...
  const coreExpense_invoice_data = await core_approval_inventory_invoice_model.aggregate([
    {
      $match: {
        _id: new mongoose.Types.ObjectId(document_id),
        invoice_id: new mongoose.Types.ObjectId(invoice_id),
  ...
  const coreExpense_invoice = coreExpense_invoice_data?.[0];
...
export const core_approve_invoice_details = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    const invoice_details = await core_approval_inventory_invoice_model
      .findOneAndUpdate(
        {
          _id: document_id,
    ...
    const update_core_invoice = await core_inventory_invoice_details.updateOne(
      { _id: invoice_id },
      {
        $set: {
          ...invoiceData,
    ...
    if (!update_core_invoice.acknowledged || update_core_invoice.modifiedCount <= 0)
      return next(new ApiError("Failed to approve invoice"), 400);

    const items_details = await core_approval_inventory_items_model
      .find({
    ...
    await core_inventory_items_details.deleteMany(
      {
        invoice_id: new mongoose.Types.ObjectId(invoice_id),
      },
      { session }
    ...
    const approval_invoice_items_data = await core_approval_inventory_items_model.aggregate([
      {
        $match: {
          approval_invoice_id: invoice_details?._id,
          invoice_id: new mongoose.Types.ObjectId(invoice_id),
      ...
      {
        $unset: ["core_item_id", "approval_invoice_id"],
      },
    ]);

    await core_inventory_items_details.insertMany(approval_invoice_items_data, { session });
...
export const core_reject_invoice_details = catchAsync(async (req, res, next) => {
  const session = await mongoose.startSession();
  session.startTransaction();
  try {
    const invoiceId = req.params?.invoice_id;
