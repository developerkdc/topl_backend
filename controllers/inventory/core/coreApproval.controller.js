...
export const coreApproval_invoice_listing = catchAsync(
  async function (req, res, next) {
    ...
    const totalCount =
      await core_approval_inventory_invoice_model.countDocuments({
        ...match_query,
      });
...
export const coreApproval_item_listing_by_invoice = catchAsync(
  async (req, res, next) => {
    ...
    const coreExpense_invoice_data =
      await core_approval_inventory_invoice_model.aggregate([
        {
          $match: {
            _id: new mongoose.Types.ObjectId(document_id),
            invoice_id: new mongoose.Types.ObjectId(invoice_id),
    ...
    const coreExpense_item_by_invoice =
      await core_approval_inventory_items_model.aggregate(aggregate_stage);
...
export const core_approve_invoice_details = catchAsync(
  async (req, res, next) => {
    ...
    try {
      ...
      if (
        !update_core_invoice.acknowledged ||
        update_core_invoice.modifiedCount <= 0
      )
      ...
      const approval_invoice_items_data =
        await core_approval_inventory_items_model.aggregate([
          {
            $match: {
              approval_invoice_id: invoice_details?._id,
              invoice_id: new mongoose.Types.ObjectId(invoice_id),
      ...
      await core_inventory_items_details.insertMany(
...
export const core_reject_invoice_details = catchAsync(
  async (req, res, next) => {
    const session = await mongoose.startSession();
    session.startTransaction();
    try {
