...
export const coreApproval_item_listing_by_invoice = catchAsync(
    async (req, res, next) => {
        ...
        const coreExpense_invoice = await core_approval_inventory_invoice_model.findOne({
            _id: document_id,
...
export const core_approve_invoice_details = catchAsync(async (req, res, next) => {
    ...
    try {
        ...
        const invoice_details = await core_approval_inventory_invoice_model.findOneAndUpdate({
            _id: document_id,
        ...
        const update_core_invoice = await core_inventory_invoice_details.updateOne({ _id: invoice_id }, {
            $set: {
                ...invoiceData,
            }
        }, { session })
        if (!update_core_invoice.acknowledged || update_core_invoice.modifiedCount <= 0) return next(new ApiError("Failed to approve invoice"), 400);

        const items_details = await core_approval_inventory_items_model.find({
...
export const core_reject_invoice_details = catchAsync(async (req, res, next) => {
    ...
    try {
        ...
        const update_core_invoice = await core_inventory_invoice_details.updateOne({ _id: invoiceId }, {
            $set: {
                approval_status: {
                    sendForApproval: {
                        status: false,
        ...
        if (!update_core_invoice.acknowledged || update_core_invoice.modifiedCount <= 0) return next(new ApiError("Failed to reject invoice"), 400);
