...
export const logApproval_invoice_listing = catchAsync(
  async function (req, res, next) {
    ...
    const totalCount =
      await log_approval_inventory_invoice_model.countDocuments({
        ...match_query,
      });
...
export const logApproval_item_listing_by_invoice = catchAsync(
  async (req, res, next) => {
    ...
    const approval_invoice_details = await log_approval_inventory_invoice_model
      .findOne({
    ...
    const logExpense_invoice_data =
      await log_approval_inventory_invoice_model.aggregate([
        {
          $match: {
            _id: new mongoose.Types.ObjectId(document_id),
            invoice_id: new mongoose.Types.ObjectId(invoice_id),
    ...
    const logExpense_item_by_invoice =
      await log_approval_inventory_items_model.aggregate(aggregate_stage);
...
export const log_approve_invoice_details = catchAsync(
  async (req, res, next) => {
    ...
    try {
      ...
      const invoice_details = await log_approval_inventory_invoice_model
        .findOneAndUpdate(
          {
            _id: document_id,
      ...
      const update_log_invoice = await log_inventory_invoice_model.updateOne(
        { _id: invoice_id },
        {
          $set: {
            ...invoiceData,
      ...
      const items_details = await log_approval_inventory_items_model
        .find({
      ...
      await log_inventory_items_model.insertMany(approval_invoice_items_data, {
        session,
      });
