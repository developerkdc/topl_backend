...
export const issue_for_grouping_from_smoking_dying_done = catchAsync(async (req, res, next) => {
    ...
    try {
        ...
        const update_process_done_items = await process_done_items_details_model.updateMany(
            {
...
export const issue_for_grouping_from_dressing_done = catchAsync(async (req, res, next) => {
    ...
    try {
        ...
        if (update_dressing_done_editable.matchedCount <= 0) {
            throw new ApiError('Failed to update dressing done editable', StatusCodes.NOT_FOUND);
        };
        if (!update_dressing_done_editable.acknowledged || update_dressing_done_editable.matchedCount <= 0) {
            throw new ApiError('Failed to update dressing done editable', StatusCodes.INTERNAL_SERVER_ERROR);
        };

        // update dressing done item issue status to grouping 
        const update_dressing_done_items = await dressing_done_items_model.updateMany(
            {
                _id: { $in: issue_for_grouping_data?.bundles },
        ...
        if (update_dressing_done_items.matchedCount <= 0) {
            throw new ApiError('Failed to update dressing done items issue status', StatusCodes.NOT_FOUND);
        }
        if (!update_dressing_done_items.acknowledged || update_dressing_done_items.matchedCount <= 0) {
            throw new ApiError('Failed to update dressing done items issue status', StatusCodes.INTERNAL_SERVER_ERROR);
        }
        ...
        if (!dressing_done_history?.[0]) {
            throw new ApiError('Failed to add dressing done history', StatusCodes.NOT_FOUND);
        }
...
export const revert_issue_for_grouping = catchAsync(async (req, res, next) => {
    ...
    try {
        ...
        const { id } = req.params;
        if (!id) {
            throw new ApiError("issue for grouping id is required", StatusCodes.BAD_REQUEST);
        }

        const issue_for_grouping = await issues_for_grouping_model.findOne({ _id: id }).lean();
        ...
        const revert_to_process_done = async function () {
            ...
            const delete_process_done_history = await process_done_history_model.deleteMany({
                process_done_id: process_done_id,
                bundles: { $all: process_done_items_ids },
            }, { session });

            if (!delete_process_done_history?.acknowledged || delete_process_done_history?.deletedCount <= 0) {
                throw new ApiError('Failed to delete process done history', StatusCodes.NOT_FOUND);
            }

            // Check is process done is editable
            const is_process_done_editable = await process_done_items_details_model.find({
        ...
        const revert_to_dressing_done = async function () {
            ...
            const delete_dressing_done_history = await dressing_done_history_model.deleteMany({
                dressing_done_other_details_id: dressing_done_id,
                bundles: { $all: dressing_done_items_ids },
            }, { session });

            if (!delete_dressing_done_history?.acknowledged || delete_dressing_done_history?.deletedCount <= 0) {
                throw new ApiError('Failed to delete dressing done history', StatusCodes.NOT_FOUND);
            }

            // Check is dressing done is editable
            const is_dressing_done_editable = await dressing_done_other_details_model.find({
        ...
        if (issue_for_grouping?.process_done_id && issue_for_grouping.issued_from === issues_for_status.smoking_dying) {
            await revert_to_process_done();
        } else if (issue_for_grouping?.dressing_done_id && issue_for_grouping.issued_from === issues_for_status.dressing) {
        ...
        if (!delete_issue_for_grouping?.acknowledged || delete_issue_for_grouping?.deletedCount <= 0) {
            throw new ApiError('Failed to delete dressing done history', StatusCodes.NOT_FOUND);
        }
