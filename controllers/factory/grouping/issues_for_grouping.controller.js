...
export const issue_for_grouping_from_veneer_inventory = catchAsync(
  async (req, res, next) => {
    ...
    try {
      ...
      const fetch_veneer_inventory_item_details =
        await veneer_inventory_items_model.find({
          _id: { $in: veneer_inventory_item_ids },
          issue_status: null,
        });
      if (
        !fetch_veneer_inventory_item_details ||
        fetch_veneer_inventory_item_details?.length <= 0
      ) {
        throw new ApiError(
          'veneer inventory item not found',
      ...
      const issue_for_grouping_details =
        fetch_veneer_inventory_item_details?.map((e) => {
          return {
      ...
      const add_issues_for_grouping =
        await issues_for_grouping_model.insertMany(issue_for_grouping_details, {
          session,
        });
      ...
      const veneer_invoice_ids = add_issues_for_grouping?.map(
        (e) => e.veneer_inventory_invoice_id
      );
      const veneer_items_ids = add_issues_for_grouping?.map(
        (e) => e.veneer_inventory_item_id
      );
      ...
      const update_veneer_invoice_editable =
...
export const issue_for_grouping_from_smoking_dying_done = catchAsync(
  async (req, res, next) => {
    ...
    try {
      ...
      const unique_identifier = new mongoose.Types.ObjectId();
      const issue_for_grouping_details = process_done_items_details?.map(
        (e) => {
          return {
            unique_identifier: unique_identifier,
      ...
      const process_done_items_id_from_issue_for_grouping =
        add_issues_for_grouping?.map((e) => e.process_done_item_id);
...
export const issue_for_grouping_from_dressing_done = catchAsync(
  async (req, res, next) => {
    ...
    try {
      ...
      const dressing_done_items_id_from_issue_for_grouping =
        add_issues_for_grouping?.map((e) => e.dressing_done_item_id);
...
export const revert_issue_for_grouping = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    if (!unique_identifier || !pallet_number) {
      throw new ApiError(
        `Please provide unique_identifier or pallet_number`,
        StatusCodes.BAD_REQUEST
      );
    }
    ...
    const revert_to_process_done = async function () {
      const process_done_id = issue_for_grouping?.bundles_details?.map(
        (e) => e.process_done_id
      );
      const process_done_items_ids = issue_for_grouping?.bundles_details?.map(
        (e) => e.process_done_item_id
      );
      ...
      const is_process_done_editable = await process_done_items_details_model
        .find({
    ...
    const revert_to_dressing_done = async function () {
      const dressing_done_id = issue_for_grouping?.bundles_details?.map(
    ...
    const revert_to_veneer_inventory = async function () {
      const veneer_inventory_invoice_ids =
        issue_for_grouping?.bundles_details?.map(
          (e) => e.veneer_inventory_invoice_id
        );
      const veneer_inventory_item_ids =
        issue_for_grouping?.bundles_details?.map(
          (e) => e.veneer_inventory_item_id
        );
    ...
    if (issue_for_grouping.issued_from === issues_for_status.smoking_dying) {
      await revert_to_process_done();
    } else if (issue_for_grouping.issued_from === issues_for_status.dressing) {
      await revert_to_dressing_done();
    } else if (issue_for_grouping.issued_from === issues_for_status.veneer) {
      await revert_to_veneer_inventory();
    } else {
      throw new ApiError('No data found to revert', StatusCodes.BAD_REQUEST);
    }

    const delete_issue_for_grouping =
      await issues_for_grouping_model.deleteMany(
        {
