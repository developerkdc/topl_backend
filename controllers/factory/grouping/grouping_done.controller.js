...
export const add_grouping_done = catchAsync(async (req, res, next) => {
    ...
    try {
        ...
        // issue for grouping
        const fetch_issue_for_grouping_data = await issues_for_grouping_model.findOne(
        ...
        const add_other_details_data =
            await grouping_done_details_model.create(
                [
                    {
                        ...other_details,
        ...
        const add_items_details_data = await grouping_done_items_details_model.insertMany(
            items_details_data,
            { session }
        );
        ...
        if (!update_issue_for_grouping.acknowledged || update_issue_for_grouping.modifiedCount <= 0) {
...
export const edit_grouping_done = catchAsync(async (req, res, next) => {
    ...
    try {
        ...
        const fetch_other_details_data = await grouping_done_details_model.findOne({ _id: grouping_done_id });
        ...
        const add_other_details_data = await grouping_done_details_model.findOneAndUpdate(
        ...
        const delete_items_details = await grouping_done_items_details_model.deleteMany(
        ...
        await session.commitTransaction();
        const response = new ApiResponse(201, 'Grouping Done Updated Successfully', {
            other_details: other_details_data,
            items_details: add_items_details_data,
        });
...
export const fetch_all_grouping_done_items = catchAsync(async (req, res, next) => {
    const {
        page = 1,
        sortBy = 'updatedAt',
        sort = 'desc',
    ...
    const result = await grouping_done_items_details_model.aggregate(list_aggregate);
...
export const revert_all_pending_done = catchAsync(async (req, res, next) => {
    ...
    try {
        ...
        const grouping_done_other_details = await grouping_done_details_model.findOne({ _id: id });
        ...
        const delete_grouping_done_other_details = await grouping_done_details_model.findOneAndDelete(
        ...
        //delete items
        const deleted_grouping_done_other_details_id = delete_grouping_done_other_details?._id;

        const delete_grouping_done_items_details = await grouping_done_items_details_model.deleteMany(
            {
                grouping_done_other_details_id: deleted_grouping_done_other_details_id,
            },
            { session }
        );
        ...
        const issue_for_grouping_id = delete_grouping_done_other_details?.issue_for_grouping_id;
...
//Damaged
export const fetch_all_damaged_grouping_done_items = catchAsync(async (req, res, next) => {
    const {
        page = 1,
        sortBy = 'updatedAt',
        sort = 'desc',
...
export const marked_as_grouping_done_damaged = catchAsync(async (req, res, next) => {
    ...
    const grouping_done_other_details = await grouping_done_items_details_model.findOne({ _id: id });

    if (!grouping_done_other_details) {
        throw new ApiError('Not data found', 400);
    }

    const update_grouping_done_damaged = await grouping_done_items_details_model.updateOne(
        { _id: id },
        {
            $set: {
                is_damaged: true,
    ...
    if (!update_grouping_done_damaged.acknowledged || update_grouping_done_damaged.modifiedCount <= 0) {
...
export const revert_grouping_done_damaged = catchAsync(async (req, res, next) => {
    ...
    const grouping_done_other_details = await grouping_done_items_details_model.findOne({ _id: id, is_damaged: true });
...
//re-create grouping
export const recreate_grouping_done_items = catchAsync(async (req, res, next) => {
    const session = await mongoose.startSession();
    session.startTransaction();
    try {
        const { id } = req.params;
        ...
        if (grouping_done_other_details.issue_status) {
            throw new ApiError(`Item already issues for ${grouping_done_other_details.issue_status}`, 400);
        }

        // item details
        const add_other_details_id = grouping_done_other_details?.grouping_done_other_details_id;
        const items_details_data = items_details?.map((item, index) => {
            item.grouping_done_other_details_id = add_other_details_id;
            item.created_by = userDetails?._id;
            item.updated_by = userDetails?._id;
        ...
        const delete_grouping_done_item = await grouping_done_items_details_model.deleteOne({ _id: grouping_done_other_details?._id }, { session });

        if (!delete_grouping_done_item.acknowledged || delete_grouping_done_item.deletedCount <= 0) {
            throw new ApiError('Failed to delete grouping done item', 400);
        }

        await session.commitTransaction();
        const response = new ApiResponse(201, 'Recreate Grouping Done Successfully', {
            items_details: add_items_details_data,
        });
