...
export const listing_canvas_damage = catchAsync(async (req, res) => {
  ...
  const canvas_damage_list = await canvas_damage_model.aggregate(listAggregate);
  ...
  const [totalDocument] = await canvas_damage_model.aggregate(totalAggregate);
...
export const add_canvas_damage = catchAsync(async (req, res) => {
  ...
  try {
    ...
    const damage_sqm = Number(
      (
        (damage_sheets / canvas_done_details?.available_details?.no_of_sheets) *
        canvas_done_details?.available_details?.sqm
      )?.toFixed(3)
    );

    const damage_amount = Number(
      ((damage_sheets / canvas_done_details?.available_details?.no_of_sheets) *
        canvas_done_details?.available_details?.amount)?.toFixed(2)
    );
      ...
      [
        {
          ...
          product_type: canvas_done_details?.product_type,
          series_code: canvas_done_details?.series_code,
    ...
    const update_canvas_done_result = await canvas_done_details_model.updateOne(
      { _id: canvas_done_details?._id },
      {
        $inc: {
    ...
    if (
      !update_canvas_done_result.acknowledged ||
      update_canvas_done_result.modifiedCount === 0
    ) {
      throw new ApiError(
        'Failed to update canvas done details.',
        StatusCodes.BAD_REQUEST
      );
    }
    ...
    const response = new ApiResponse(
      StatusCodes.OK,
      'canvas Item added to damage successfully.',
...
export const revert_damage_to_canvas_done = catchAsync(async (req, res) => {
  ...
  try {
    ...
    const delete_damage_data_result = await canvas_damage_model.deleteOne(
      { _id: id },
      { session }
    );
    ...
    const update_canvas_done_item_result =
      await canvas_done_details_model.findOneAndUpdate(
        { _id: canvas_damage_details?.canvas_done_id },
        {
          $inc: {
            'available_details.no_of_sheets':
              canvas_damage_details.no_of_sheets,
            'available_details.sqm': canvas_damage_details.sqm,
            'available_details.amount': canvas_damage_details.amount,
          },
        },
        { session }
      );
    ...
    if (
      ...
      await canvas_done_details_model.updateOne(
