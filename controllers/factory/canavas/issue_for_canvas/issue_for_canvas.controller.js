...
import {
  issue_for_canvas_model,
  issue_for_canvas_view_model,
} from '../../../../database/schema/factory/canvas/issue_for_canvas/issue_for_canvas.schema.js';
import { issues_for_status } from '../../../../database/Utils/constants/constants.js';
import { createFactoryIssueForCanvasExcel } from '../../../../config/downloadExcel/Logs/Factory/Canvas/IssueForCanvas/index.js';
...
export const revert_issue_for_canvas = catchAsync(async (req, res) => {
  ...
  try {
    ...
    const update_pressing_details_result =
      await pressing_item_details_model?.updateOne(
        { _id: issue_for_canvas_details?.pressing_details_id },
        {
          $inc: {
    ...
    const delete_issue_for_canvas_document_result =
      await issue_for_canvas_model.deleteOne(
        { _id: issue_for_canvas_details?._id },
        { session }
      );

    if (
      !delete_issue_for_canvas_document_result?.acknowledged ||
      delete_issue_for_canvas_document_result?.deletedCount === 0
    ) {
      throw new ApiError(
        'Failed to delete issue for canvas details.',
        StatusCodes.BAD_REQUEST
      );
    }
...
export const listing_issued_for_canvas = catchAsync(async (req, res, next) => {
  ...
  const totalDocument =
    await issue_for_canvas_view_model.aggregate(totalAggregate);
...
export const fetch_single_issue_for_canvas_item = catchAsync(
  async (req, res) => {
    const { id } = req.params;
    ...
    const result = await issue_for_canvas_view_model
      .findOne({ _id: id })
      .lean();

    if (!result) {
      throw new ApiError(
        'Issue for canvas data not found',
        StatusCodes.NOT_FOUND
      );
    }
...
export const download_excel_issued_for_canvas = catchAsync(async (req, res, next) => {
  const {
    page = 1,
    limit = 10,
    sortBy = 'updatedAt',
  ...
  const data = await issue_for_canvas_view_model.aggregate(listAggregate);
  await createFactoryIssueForCanvasExcel(data,req,res);
});