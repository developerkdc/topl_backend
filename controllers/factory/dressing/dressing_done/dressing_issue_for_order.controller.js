...
export const fetch_all_log_no_code_by_item_name = catchAsync(async (req, res) => {
    const { id } = req.params;
    ...
    const result = await dressing_done_items_model
        ?.aggregate(pipeline)
        .collation({ caseLevel: true, locale: 'en' });

    const response = new ApiResponse(
        StatusCodes.OK,
        'Log No Code Dropdown fetched successfully',
        result
    );
    return res.status(StatusCodes.OK).json(response);
});

export const fetch_all_pallet_no_by_log_no_code = catchAsync(async (req, res) => {
    const { log_no_code } = req.params;
...
export const fetch_all_bundles_by_pallet_number = catchAsync(async (req, res) => {
    ...
    const result = await dressing_done_items_model.aggregate([match__query, aggProject]);
    const response = new ApiResponse(
        StatusCodes.OK,
        'Bundle No Dropdown fetched successfully',
        result
    );
    return res.status(StatusCodes.OK).json(response);
});
export const fetch_dressing_done_details_by_bundle_id = catchAsync(async (req, res) => {
    const { id } = req.params;
    ...
    const dressing_done_item_details = await dressing_done_items_model.findById(id);
    const response = new ApiResponse(
        StatusCodes.OK,
        'Dressing Done Item Details fetched successfully',
        dressing_done_item_details
    );
    return res.status(StatusCodes.OK).json(response);
});
