...
export const add_peeling_done = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    const issue_for_peeling_type = await issues_for_peeling_model.updateOne(
    ...
    if (issue_for_peeling_type?.type === issue_for_peeling.re_flitching) {
      ...
      const add_re_flitching_details_data =
      ...
      if (add_re_flitching_details_data?.length <= 0) {
        throw new ApiError('Failed to add re flitching details', 400);
      }
    }
