...
export const add_peeling_done = catchAsync(async (req, res, next) => {
    ...
    try {
        ...
        const { other_details, items_details, type, wastage_details, available_details } = req.body;
        for (let i of ["other_details", "items_details", "type", "wastage_details", "available_details"]) {
            if (!req.body?.[i]) {
        ...
        const issue_for_peeling_type = await issues_for_peeling_model.updateOne({ _id: issue_for_peeling_id }, {
            $set: {
                type: type,
                updated_by: userDetails?._id,
            }
        ...
        if (issue_for_peeling_type?.type === issue_for_peeling.wastage) {
            ...
            const add_wastage_details_data = await issues_for_peeling_wastage_model.create([wastage_details_data], {
                session,
            });
        ...
        if (issue_for_peeling_type?.type === issue_for_peeling.re_flitching) {
            ...
            const add_rejection_details_data = await issues_for_peeling_available_model.create([re_flitching_details_data], {
                session,
            });
            if (add_rejection_details_data?.length <= 0) {
