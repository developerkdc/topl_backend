...
export const addIssueForPeelingFromLogInventory = catchAsync(async function (req, res, next) {
    ...
    try {
        ...
        if (!Array.isArray(log_inventory_item_ids) || log_inventory_item_ids.length === 0) {
            return next(new ApiError("Log inventory item must be a array field"))
        }

        const logInventoryItemData = await log_inventory_items_view_model.find({ _id: { $in: log_inventory_item_ids }, issue_status: issues_for_status.log }).lean();

        if (logInventoryItemData?.length <= 0) {
            return next(new ApiError("Log Inventory Item Data Not Found", StatusCodes.NOT_FOUND));
        }

        const issue_for_peeling_data = logInventoryItemData?.map((logInventoryItem) => {
            return {
                ...
                invoice_date: logInventoryItem?.log_invoice_details?.invoice_Details?.invoice_date,
                invoice_no: logInventoryItem?.log_invoice_details?.invoice_Details?.invoice_no,
        ...
        const add_issue_for_peeling = await issue_for_peeling_model.insertMany(issue_for_peeling_data, { session });

        if (add_issue_for_peeling?.length <= 0) {
            return next(new ApiError("Failed to data for issue for peeling", 400))
        }
        const log_item_ids = add_issue_for_peeling?.map((ele) => ele?.log_inventory_item_id)
        const log_invoice_ids = [...new Set(add_issue_for_peeling.map(issue => issue.invoice_id))]

        //updating log inventory item status to peeling
        const update_log_inventory_item_status = await log_inventory_items_model.updateMany({ _id: { $in: log_item_ids } }, {
            $set: {
        ...
        if (!update_log_inventory_item_status.acknowledged || update_log_inventory_item_status?.modifiedCount <= 0) {
            return next(new ApiError("Unable to change status of log inventory item"))
        }

        //updating log inventory invoice: if any one of log item send for peeling then invoice should not editable
        const update_log_inventory_invoice_editable = await log_inventory_invoice_model.updateMany(
        ...
        if (!update_log_inventory_invoice_editable.acknowledged || update_log_inventory_invoice_editable?.modifiedCount <= 0) {
            return next(new ApiError("Unable to change status of log inventory invoice"))
        }
