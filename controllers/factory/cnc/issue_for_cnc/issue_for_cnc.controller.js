...
export const add_issue_for_cnc_from_pressing = catchAsync(async (req, res) => {
    ...
    try {
        ...
        const max_sr_no_doc = await issue_for_cnc_model
            .findOne({}, { sr_no: 1 })
            .sort({ sr_no: -1 })
            .limit(1)
            .lean();
        ...
        const [add_issue_for_cnc_result] = await issue_for_cnc_model.create(
            [
                {
                    sr_no: max_sr_no,
        ...
        if (!add_issue_for_cnc_result) {
            throw new ApiError(
                'Failed to add issue for cnc data.',
        ...
        const [create_pressing_history] = await pressing_item_details?.create([{
            issue_status: issues_for_status?.cnc_factory,
        ...
        const response = new ApiResponse(
            StatusCodes.OK,
            'Item Issued For CNC Successfully',
            add_issue_for_cnc_result
        );
...
export const revert_issue_for_cnc = catchAsync(async (req, res) => {
    const { id } = req.params;
    ...
    try {
        ...
        const issue_for_cnc_details = await issue_for_cnc_model.findById(id).lean();
        ...
        const delete_issue_for_cnc_document_result = await issue_for_cnc_model.deleteOne({ _id: issue_for_cnc_details?._id }, { session });

        if (!delete_issue_for_cnc_document_result?.acknowledged || delete_issue_for_cnc_document_result?.deletedCount === 0) {
            throw new ApiError("Failed to delete issue for cnc details.", StatusCodes.BAD_REQUEST);
        }
...
export const listing_issued_for_cnc = catchAsync(async (req, res, next) => {
    ...
    const issue_for_cnc =
        await issue_for_cnc_model.aggregate(listAggregate);
    ...
    const totalDocument =
        await issue_for_cnc_model.aggregate(totalAggregate);
