...
export const create_cnc = catchAsync(async (req, res) => {
    ...
    try {
        ...
        if (!isValidObjectId(cnc_done_details?.issur_for_cnc_id)) {
...
export const revert_cnc_done_items = catchAsync(async (req, res) => {
    ...
    try {
        ...
        const response = new ApiResponse(StatusCodes.OK, "CNC items Reverted Successfully", delete_cnc_done_result)
        await session.commitTransaction();
        return res.status(StatusCodes.OK).json(response)
    } catch (error) {
        await session?.abortTransaction()
