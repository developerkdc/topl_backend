...
export const revert_issue_for_tapping_item = catchAsync(async (req, res, next) => {
    const session = await mongoose.startSession();
    session.startTransaction();
    try {
        const { issue_for_tapping_id } = req.params;
        const userDetails = req?.userDetails;
        if (!issue_for_tapping_id || !mongoose.isValidObjectId(issue_for_tapping_id)) {
            throw new ApiError('Invalid issue for tapping id', StatusCodes.BAD_REQUEST);
        }

        const fetch_issue_for_tapping_item_details = await issue_for_tapping_model.findOne({ _id: issue_for_tapping_id }).lean();
        if (!fetch_issue_for_tapping_item_details) {
            throw new ApiError('Failed to find issue for tapping item details', StatusCodes.NOT_FOUND);
        }

        // delete issue for tapping item
        const delete_issue_for_tapping_item = await issue_for_tapping_model.findOneAndDelete(
            { _id: fetch_issue_for_tapping_item_details?._id },
            { session }
        ).lean();
        if (!delete_issue_for_tapping_item) {
            throw new ApiError('Failed to delete issue for tapping item', StatusCodes.INTERNAL_SERVER_ERROR);
        };
        ...
        const delete_grouping_done_history_item = await grouping_done_history_model.deleteOne(
            {
        ...
        if (!delete_grouping_done_history_item.acknowledged || delete_grouping_done_history_item.deletedCount <= 0) {
            throw new ApiError('Failed to delete grouping done history item', StatusCodes.INTERNAL_SERVER_ERROR);
        };
        ...
        const update_grouping_done_item_details = await grouping_done_items_details_model.updateOne(
...
export const fetch_single_issue_for_tapping_details = catchAsync(async (req, res, next) => {
    const { id } = req.params;
    if (!id) {
...
export const fetch_all_issue_for_tapping_details = catchAsync(async (req, res, next) => {
    const {
        page = 1,
        limit = 10,
        sortBy = 'updatedAt',
