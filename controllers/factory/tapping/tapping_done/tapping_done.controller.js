...
export const add_tapping_details = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    const { other_details, items_details, is_wastage, wastage_details } =
      req.body;
    ...
    if (!Array.isArray(items_details)) {
      throw new ApiError(
        'items_details must be array',
        StatusCodes.BAD_REQUEST
      );
    }
    if (items_details?.length < 0) {
      throw new ApiError(
        'Atleast one items is required',
        StatusCodes.BAD_REQUEST
      );
    }
...
export const edit_tapping_details = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    // Other goods details
    const { _id, createdAt, updatetAt, ...update_other_details } =
      other_details;
    const add_other_details_data =
      await tapping_done_other_details_model.findOneAndUpdate(
        { _id: tapping_done_id },
        {
          $set: {
    ...
    if (!other_details_data) {
      throw new ApiError(
        'Failed to update other details',
        StatusCodes.BAD_REQUEST
      );
    }
    ...
    if (
      ...
      throw new ApiError(
        'Failed to delete items details',
        StatusCodes.BAD_REQUEST
      );
    }
    ...
    if (is_wastage) {
      ...
      if (!add_wastage_details_data) {
        throw new ApiError(
          'Failed to update wastage details',
          StatusCodes.BAD_REQUEST
        );
      }
    }
