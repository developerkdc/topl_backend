...
import { createFactoryIssueForPolishExcel } from '../../../../config/downloadExcel/Logs/Factory/Polish/IssueForPolish/index.js';
...
export const listing_issued_for_polishing = catchAsync(
  async (req, res, next) => {
    ...
    const optimizedResult = await runConditionalHydratedPagination({
      model: issue_for_polishing_model,
      hydrateModel: issue_for_polishing_view_model,
      req,
      staticMatch: {
      ...
      fallbackRunner: async ({
        ...
        return {
          ...
          totalPages: Math.ceil(
            (totalDocument?.[0]?.totalCount || 0) / pageLimit
          ),
        };
      },
    });
