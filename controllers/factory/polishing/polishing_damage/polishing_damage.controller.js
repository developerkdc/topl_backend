...
export const add_polishing_damage = catchAsync(async (req, res) => {
  ...
  try {
    ...
    const polishing_done_details = await polishing_done_details_model
      .findById(id)
      .lean()
      .session();
    ...
    const damage_sqm = Number(
      (
        (damage_sheets /
          polishing_done_details?.available_details?.no_of_sheets) *
        polishing_done_details?.available_details?.sqm
      )?.toFixed(3)
    );

    const damage_amount = Number(
      ((damage_sheets / polishing_done_details?.available_details?.no_of_sheets) *
        polishing_done_details?.available_details?.amount)?.toFixed(2)
    );
    ...
    const update_polishing_done_result =
      await polishing_done_details_model.updateOne(
        { _id: polishing_done_details?._id },
        {
          $inc: {
    ...
    if (
      !update_polishing_done_result.acknowledged ||
      update_polishing_done_result.modifiedCount === 0
    ) {
      throw new ApiError(
        'Failed to update polishing done details.',
        StatusCodes.BAD_REQUEST
      );
    }
    ...
    const response = new ApiResponse(
      StatusCodes.OK,
      'polishing Item added to damage successfully.',
...
export const revert_damage_to_polishing_done = catchAsync(async (req, res) => {
  ...
  try {
    ...
    const polishing_damage_details = await polishing_damage_model
      .findById(id)
      .lean()
      .session(session);
    if (!polishing_damage_details) {
      throw new ApiError(
        'polishing Damage details not found',
        StatusCodes.NOT_FOUND
      );
    }

    const delete_damage_data_result = await polishing_damage_model.deleteOne(
      { _id: id },
      { session }
    );
    ...
    const update_polishing_done_item_result =
      await polishing_done_details_model.findOneAndUpdate(
        { _id: polishing_damage_details?.polishing_done_id },
        {
          $inc: {
            'available_details.no_of_sheets':
              polishing_damage_details.no_of_sheets,
            'available_details.sqm': polishing_damage_details.sqm,
            'available_details.amount': polishing_damage_details.amount,
          },
        },
        { session }
      );
    ...
    const is_item_editable = await polishing_done_details_model
      .findById(polishing_damage_details?.polishing_done_id)
      .lean()
      .session(session);
