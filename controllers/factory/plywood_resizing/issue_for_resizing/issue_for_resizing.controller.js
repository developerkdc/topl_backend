...
export const add_issue_for_resizing_from_plywood = catchAsync(
  async (req, res) => {
    ...
    try {
      ...
      const factor = Number(
        issued_sheets / plywood_item_details?.available_sheets
      );
      const issued_sqm = Number(
        (factor * plywood_item_details?.available_sqm)?.toFixed(3)
      );
      const issued_amount = Number(
        (factor * plywood_item_details?.available_amount).toFixed(2)
      );
      ...
      const insert_issue_for_resize_result =
        await issue_for_plywood_resizing_model.create([issue_for_resize_data], {
          session,
        });
      if (insert_issue_for_resize_result?.length === 0) {
        throw new ApiError(
          'Failed to add issue for resize data',
          StatusCodes.BAD_REQUEST
        );
      }

      const plywood_inventory_item_id =
        insert_issue_for_resize_result?.[0]?.plywood_item_id;
      const issue_for_resizing_id = insert_issue_for_resize_result?.[0]?._id;

      const update_inventory_item_issue_status_result =
        await plywood_inventory_items_details.updateOne(
          { _id: plywood_inventory_item_id },
          {
            $inc: {
      ...
      if (
        !update_inventory_item_issue_status_result?.acknowledged ||
        update_inventory_item_issue_status_result?.modifiedCount === 0
      ) {
        throw new ApiError(
          'Failed to update plywood item issue status',
          StatusCodes.BAD_REQUEST
        );
      }
      const update_inventory_item_invoice_editable_status_result =
        await plywood_inventory_invoice_details.updateOne(
          { _id: plywood_item_details?.invoice_id },
          {
            $set: {
              isEditable: false,
              updated_by: userDetails?._id,
      ...
      if (
        update_inventory_item_invoice_editable_status_result?.matchedCount === 0
      ) {
        throw new ApiError(
          'Plywood items invoice not found',
          StatusCodes.BAD_REQUEST
        );
      }
      if (
        !update_inventory_item_invoice_editable_status_result?.acknowledged ||
        update_inventory_item_invoice_editable_status_result.modifiedCount === 0
      ) {
        throw new ApiError(
          'Failed to update plywood items invoice status',
          StatusCodes.BAD_REQUEST
        );
      }
      ...
      if (add_plywood_history?.length === 0) {
        throw new ApiError(
          'Failed to create plywood item history',
          StatusCodes.BAD_REQUEST
        );
      }
      const response = new ApiResponse(
        ...
        insert_issue_for_resize_result
      );
...
export const revert_issue_for_resizing = catchAsync(async (req, res) => {
  ...
  try {
    ...
    const update_plywood_inventory_item_status_result =
      await plywood_inventory_items_details.findOneAndUpdate(
        { _id: resizing_item_details?.plywood_item_id },
        {
          $inc: {
    ...
    const delete_issue_for_resizing_document_result =
      await issue_for_plywood_resizing_model.deleteOne(
        { _id: resizing_item_details?._id },
        { session }
      );

    if (
      !delete_issue_for_resizing_document_result?.acknowledged ||
      delete_issue_for_resizing_document_result?.deletedCount === 0
    ) {
      throw new ApiError(
        'Failed to delete issue for resizing details',
        StatusCodes.BAD_REQUEST
      );
    }
    const delete_plywood_history = await plywood_history_model.deleteOne(
      { issued_for_plywood_resizing_id: resizing_item_details?._id },
      { session }
    );

    if (
      !delete_plywood_history.acknowledged ||
      delete_plywood_history.deletedCount === 0
    ) {
      throw new ApiError(
    ...
    const response = new ApiResponse(
      ...
      delete_issue_for_resizing_document_result
    );
    await session.commitTransaction();
    return res.status(StatusCodes.OK).json(response);
  } catch (error) {
...
export const fetch_single_issue_for_resizing_item = catchAsync(
  async (req, res) => {
    const { id } = req.params;
    ...
    const result = await issue_for_plywood_resizing_model
      .findOne({ _id: id })
      .lean();
