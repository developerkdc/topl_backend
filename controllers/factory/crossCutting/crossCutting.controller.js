...
export const revert_issue_for_crosscutting = catchAsync(async function (req, res, next) {
  ...
  try {
    ...
    return res.status(200).json(
      new ApiResponse(StatusCodes.OK, "Reverted successfully")
    );
  } catch (error) {
    console.log(error);
...
export const add_cross_cutting_inventory = catchAsync(async (req, res, next) => {
  ...
  const is_crosscutting_completed = req.body?.is_crosscutting_completed || false;
  ...
  for (let crosscutting of crosscutting_details) {
    ...
    issued_crosscuting_avaiable_physical_length = issued_crosscuting_avaiable_physical_length - crosscutting_done_length;

    //Comparing avaible physical length should not be less then 0
    if (issued_crosscuting_avaiable_physical_length < 0) return next(new ApiError("Invaild input of length", 400));
    ...
    const addCrosscutting = await crosscutting_done_model.create({
      ...
      log_inventory_item_id: issues_for_crosscutting_data?.log_inventory_item_id,
