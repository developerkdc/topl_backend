...
export const add_process_done_details = catchAsync(async (req, res, next) => {
    ...
    try {
        ...
        if (!process_done_details) {
            throw new ApiError(`Please provide process_done_details`, StatusCodes.BAD_REQUEST);
        }
        const required_field = ["issue_for_smoking_dying_unique_identifier", "issue_for_smoking_dying_pallet_number"];
        ...
        const unique_identifier = mongoose.Types.ObjectId.createFromHexString(process_done_details?.issue_for_smoking_dying_unique_identifier);
        const pallet_number = process_done_details?.issue_for_smoking_dying_pallet_number;
        ...
        if (!add_process_done_details || !add_process_done_details?.[0]) {
            throw new ApiError('No issue for smoking dying data found', StatusCodes.BAD_GATEWAY);
        };

        const process_done_details_data = add_process_done_details?.[0];

        const process_done_items_details = await issues_for_smoking_dying_model.aggregate([
            {
                $match: {
                    unique_identifier: process_done_details_data?.issue_for_smoking_dying_unique_identifier,
                    pallet_number: process_done_details_data?.issue_for_smoking_dying_pallet_number,
                }
            },
            {
                $set: {
        ...
        const add_process_done_items_details = await process_done_items_details_model.insertMany(process_done_items_details, { session });
        if (add_process_done_items_details?.length <= 0) {
            throw new ApiError('Failed to add process done data', StatusCodes.BAD_GATEWAY);
        };
...
export const edit_process_done_details = catchAsync(async (req, res, next) => {
    ...
    try {
        ...
        if (!process_done_details_id) {
            throw new ApiError(`Please provide process_done_details_id`, StatusCodes.BAD_REQUEST);
        }
        if (!process_done_details) {
            throw new ApiError(`Please provide process_done_details`, StatusCodes.BAD_REQUEST);
        }
        ...
        const update_process_done_details = await process_done_details_model.findOneAndUpdate({
            _id: process_done_details_id,
        }, {
            $set: {
                ...process_done_details,
        ...
        if (!update_process_done_details) {
            throw new ApiError('Failed to update process done', StatusCodes.BAD_GATEWAY);
        };

        const update_process_done_items_details = await process_done_items_details_model.updateMany({
        ...
        if (update_process_done_items_details?.matchedCount <= 0) {
            throw new ApiError('No process done items details found', StatusCodes.BAD_GATEWAY);
        }

        if (!update_process_done_items_details.acknowledged || update_process_done_items_details.modifiedCount <= 0) {
            throw new ApiError('Failed to update process done items details', StatusCodes.BAD_GATEWAY);
        }
...
export const fetch_single_process_done_details = catchAsync(async (req, res, next) => {
    const { process_done_details_id } = req.params;
    if (!process_done_details_id) {
        throw new ApiError(`Please provide process_done_details_id`, StatusCodes.BAD_REQUEST);
    }
    ...
    const agg_match = {
        $match: {
            _id: mongoose.Types.ObjectId.createFromHexString(process_done_details_id),
        }
    }
    ...
    const process_done_details = await process_done_details_model.aggregate(aggregation_pipeline);
...
export const fetch_all_process_done_details = catchAsync(async (req, res, next) => {
    const {
        page = 1,
        sortBy = 'updatedAt',
        sort = 'desc',
    ...
    const result = await process_done_details_model.aggregate(aggregation_pipeline);
    ...
    const total_docs = await process_done_details_model.aggregate(total_count_aggregation_pipeline);
