...
export const revert_issued_for_smoking_dying_item = catchAsync(
  async (req, res, next) => {
    ...
    try {
      ...
      const fetch_issue_for_smoking_dying_data =
        await issues_for_smoking_dying_model.findOne({ _id: id }).lean();
      if (!fetch_issue_for_smoking_dying_data) {
        throw new ApiError(
          'Issue for Smoking Dying Item Not Found',
          StatusCodes.NOT_FOUND
        );
      }

      const revert_to_veneer_inventory = async function () {
        const veneer_inventory_item_id =
          fetch_issue_for_smoking_dying_data?.veneer_inventory_id;
      ...
      if (
        fetch_issue_for_smoking_dying_data?.veneer_inventory_id !== null &&
        issues_for_status?.veneer
      ) {
        await revert_to_veneer_inventory();
      } else if (
        fetch_issue_for_smoking_dying_data?.dressing_done_id !== null &&
