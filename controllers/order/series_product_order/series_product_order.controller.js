...
export const add_series_order = catchAsync(async (req, res) => {
  ...
  try {
    ...
    const max_order_no = await OrderModel.findOne({}, { order_no: 1 }).sort({
      order_no: -1,
    });
...
export const update_series_order = catchAsync(async (req, res) => {
  ...
  try {
    ...
    if (!send_for_approval) {
      const existingOrderItemsForLock =
        await series_product_order_item_details_model.find(
          { order_id: order_details_id },
          { _id: 1 },
          { session }
        );
      const orderProcessLockedItemIds = await findProcessLockedItemIds(
        existingOrderItemsForLock.map((item) => item._id),
        session
      );
      const changedOrderFields = ORDER_LOCKED_FIELDS.filter(
        (field) =>
          Object.prototype.hasOwnProperty.call(order_details, field) &&
          hasFieldChanged(order_details_result, order_details, field)
      );
      if (orderProcessLockedItemIds.size && changedOrderFields.length) {
        throw new ApiError(
          `Cannot edit ${changedOrderFields.join(', ')} because an order item has already been issued for processing.`,
          StatusCodes.BAD_REQUEST
        );
      }
      ...
      const processLockedItemIds = await findProcessLockedItemIds(
        order_items_details.map((item) => item._id),
        session
      );
      ...
      for (const existingItem of order_items_details) {
      ...
      for (const item of order_items_details) {
        if (item.photo_number && item.photo_number_id) {
          const revertSheets = item.pressing_instructions === "BOTH SIDE WITH SAME GROUP" ? item.no_of_sheets * 2 : item.no_of_sheets;
...
export const update_series_order_item_status_by_item_id = catchAsync(
  async (req, res) => {
    const { id } = req.params;
    const { _id } = req.userDetails;

    const update_result =
      await series_product_order_item_details_model?.updateOne(
        { _id: id },
        {
          $set: {
...
export const fetch_all_series_order_items = catchAsync(
  async (req, res, next) => {
    const {
      page = 1,
      sortBy = 'updatedAt',
    ...
    const result =
      await series_product_order_item_details_model?.aggregate(list_aggregate);
    ...
    const total_docs =
      await series_product_order_item_details_model.aggregate(count_total_docs);
...
export const fetch_all_series_order_items_by_order_id = catchAsync(
  async (req, res) => {
    const { id } = req.params;
