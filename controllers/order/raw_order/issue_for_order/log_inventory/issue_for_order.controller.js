...
export const add_issue_for_order = catchAsync(async (req, res) => {
    ...
    try {
        ...
        const update_log_item_issue_status = await log_inventory_items_model.updateOne({ _id: log_item_data?._id }, {
            $set: {
        ...
        const update_log_inventory_invoice_editable_status = await log_inventory_invoice_model?.updateOne({ _id: log_item_data?.invoice_id }, {
            $set: {
                isEditable: false,
                updated_by: userDetails?._id
            }
        ...
        if (!update_log_inventory_invoice_editable_status?.acknowledged || update_log_inventory_invoice_editable_status?.modifiedCount === 0) {
            throw new ApiError("Failed to update Log item invoice status", StatusCodes.BAD_REQUEST)
        };
...
export const fetch_all_issued_logs_by_order_item_id = catchAsync(async (req, res) => {
    const { id } = req.params;
    ...
    const response = new ApiResponse(StatusCodes.OK, "Issued Items fetched successfully", result);
