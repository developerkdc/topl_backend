...
import { create_decorative_order_approval_report } from '../../../config/downloadExcel/orders/decorative/decorative_order.approval.report.js';
...
export const fetch_all_decorative_order_items = catchAsync(
  async (req, res, next) => {
    ...
    const result =
      await approval_decorative_order_item_details_model?.aggregate(
        list_aggregate
      );
    ...
    const response = new ApiResponse(
      StatusCodes?.OK,
      'Decorative Order Approval Details Fetched Successfully',
      {
        data: result,
...
export const fetch_all_decorative_order_items_by_order_id = catchAsync(
  async (req, res) => {
    ...
    const pipeline = [
      ...
      {
        $lookup: {
          from: 'approval_decorative_order_item_details',
...
export const approve_order = catchAsync(async (req, res) => {
  ...
  try {
    ...
    const existing_decorative_order_items =
      await decorative_order_item_details_model
    ...
    for (const item of existing_decorative_order_items) {
    ...
    const delete_existing_decorative_order_items =
      await decorative_order_item_details_model
        .deleteMany({ order_id: order_id }, { session })
        .lean();

    if (
      !delete_existing_decorative_order_items?.acknowledged ||
      delete_existing_decorative_order_items?.deletedCount === 0
    ) {
      throw new ApiError(
        'Failed to delete existing decorative order items',
        StatusCodes.BAD_REQUEST
      );
    }
    ...
    const response = new ApiResponse(
      StatusCodes.OK,
      'Decorative Order approved successfully',
      {
