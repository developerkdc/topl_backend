...
class RawOrderCancelController {
    ...
    static fetch_order_item_details = (async ({ order_id, order_item_id }) => {
        try {
            ...
            const [fetch_order_item_details] = await RawOrderItemDetailsModel.aggregate([
                match_query,
                lookup_order,
                unwind_order
            ]);
    ...
    static revert_items_issued_for_order = (async ({ order_id, order_item_id, other_details }, session) => {
        try {
            this.validate_fields({ order_id, order_item_id }, "order_item");
            const userDetails = other_details?.userDetails;
            const issued_for_order_item_details = await issue_for_order_model.aggregate([
                {
                    $match: {
                        order_id: mongoose.Types.ObjectId.createFromHexString(order_id),
            ...
            for (let item of issued_for_order_item_details) {
                const revert_issued_for_order_items = new RevertOrderItem(item?._id, userDetails, session);
                await revert_issued_for_order_items.update_inventory_item_status();
            }

            const delete_issued_for_order_item_details = await issue_for_order_model.deleteMany(
                {
                    order_id: order_id,
            ...
            if (
                !delete_issued_for_order_item_details.acknowledged
            ) {
                throw new ApiError(
            ...
            const cancel_order_item_status = await RawOrderItemDetailsModel.findOneAndUpdate({
                order_id: order_id,
                _id: order_item_id
            }, {
                $set: {
    ...
    static cancel_order_item = catchAsync(async (req, res, next) => {
        ...
        try {
            ...
            const order_item_details = await this.fetch_order_item_details({ order_id, order_item_id });
