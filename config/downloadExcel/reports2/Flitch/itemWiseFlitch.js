...
export const createItemWiseFlitchReportExcel = async (
  ...
  try {
    ...
    const columnDefinitions = [
      ...
      ...(includeCostAndExpense ? [
        { key: 'opening_cost_amount', width: 12 }, // 3.  Cost Amount
        { key: 'opening_expense_amount', width: 12 }, // 4.  Expense Amount
      ] : []),
      { key: 'invoice_cmt', width: 12 }, // 3.  Invoice    ┐ Round Log Detail CMT
      { key: 'indian_cmt', width: 12 }, // 4.  Indian     │
      { key: 'actual_cmt', width: 12 }, // 5.  Actual     ┘
      ...(includeCostAndExpense ? [
        { key: 'actual_cost_amount', width: 12 }, // 6.  Cost Amount
        { key: 'actual_expense_amount', width: 12 }, // 7.  Expense Amount
      ] : []),
      ...
      ...(includeCostAndExpense ? [
        { key: 'flitch_cost_amount', width: 12 }, // 10. Cost Amount
        { key: 'flitch_expense_amount', width: 12 }, // 11. Expense Amount
      ] : []),
      ...
      ...(includeCostAndExpense ? [
        { key: 'slicing_cost_amount', width: 12 }, // 13. Cost Amount
        { key: 'slicing_expense_amount', width: 12 }, // 14. Expense Amount
      ] : []),
      ...
      ...(includeCostAndExpense ? [
        { key: 'closing_cost_amount', width: 12 }, // 17. Cost Amount
        { key: 'closing_expense_amount', width: 12 }, // 18. Expense Amount
      ] : []),
    ];
    ...
    const colIndex = (key) => columnDefinitions.findIndex((c) => c.key === key) + 1; // 1-based

    // Row 3: Group header row — build dynamically
    const groupHeaderValues = new Array(TOTAL_COLS).fill('');
    groupHeaderValues[colIndex('invoice_cmt') - 1] = 'Round Log Detail CMT';
    ...
    groupHeaderRow.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFD3D3D3' } };
    applyRowBorders(groupHeaderRow, 1, TOTAL_COLS, { top: true, bottom: true });

    // Merge group header cells — using actual column positions
    const extra = includeCostAndExpense ? 2 : 0;
    worksheet.mergeCells(3, colIndex('invoice_cmt'), 3, colIndex('actual_cmt') + extra);
    worksheet.mergeCells(3, colIndex('issue_for_flitch'), 3, colIndex('flitch_diff') + extra);
    worksheet.mergeCells(3, colIndex('issue_for_slicing'), 3, colIndex('slicing_diff') + extra);
    ...
    sortedData.forEach((item) => {
      const dataRow = worksheet.addRow({
        ...
        ...(includeCostAndExpense && {
          opening_cost_amount: parseFloat(item.opening_cost_amount || 0).toFixed(3),
          opening_expense_amount: parseFloat(item.opening_expense_amount || 0).toFixed(3),
          actual_cost_amount: parseFloat(item.actual_cost_amount || 0).toFixed(3),
          actual_expense_amount: parseFloat(item.actual_expense_amount || 0).toFixed(3),
          flitch_cost_amount: parseFloat(item.flitch_cost_amount || 0).toFixed(3),
          flitch_expense_amount: parseFloat(item.flitch_expense_amount || 0).toFixed(3),
          slicing_cost_amount: parseFloat(item.slicing_cost_amount || 0).toFixed(3),
          slicing_expense_amount: parseFloat(item.slicing_expense_amount || 0).toFixed(3),
          closing_cost_amount: parseFloat(item.closing_cost_amount || 0).toFixed(3),
          closing_expense_amount: parseFloat(item.closing_expense_amount || 0).toFixed(3),
        }),
      });
      ...
      if (includeCostAndExpense) {
        grandTotals.opening_cost_amount += parseFloat(item.opening_cost_amount || 0);
        grandTotals.opening_expense_amount += parseFloat(item.opening_expense_amount || 0);
        grandTotals.actual_cost_amount += parseFloat(item.actual_cost_amount || 0);
        grandTotals.actual_expense_amount += parseFloat(item.actual_expense_amount || 0);
        grandTotals.flitch_cost_amount += parseFloat(item.flitch_cost_amount || 0);
        grandTotals.flitch_expense_amount += parseFloat(item.flitch_expense_amount || 0);
        grandTotals.slicing_cost_amount += parseFloat(item.slicing_cost_amount || 0);
        grandTotals.slicing_expense_amount += parseFloat(item.slicing_expense_amount || 0);
        grandTotals.closing_cost_amount += parseFloat(item.closing_cost_amount || 0);
        grandTotals.closing_expense_amount += parseFloat(item.closing_expense_amount || 0);
      }
    });
    ...
    const totalRow = worksheet.addRow({
      ...
      ...(includeCostAndExpense && {
        opening_cost_amount: grandTotals.opening_cost_amount.toFixed(3),
        opening_expense_amount: grandTotals.opening_expense_amount.toFixed(3),
        actual_cost_amount: grandTotals.actual_cost_amount.toFixed(3),
        actual_expense_amount: grandTotals.actual_expense_amount.toFixed(3),
        ...
        slicing_cost_amount: grandTotals.slicing_cost_amount.toFixed(3),
        slicing_expense_amount: grandTotals.slicing_expense_amount.toFixed(3),
        closing_cost_amount: grandTotals.closing_cost_amount.toFixed(3),
        closing_expense_amount: grandTotals.closing_expense_amount.toFixed(3),
      }),
    });
