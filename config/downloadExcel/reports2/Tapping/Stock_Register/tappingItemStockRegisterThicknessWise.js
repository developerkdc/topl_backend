 ...
 *   Col  5: Opening Balance  (merged rows 1–2, or block: Qty|Amount|Expense Amount)
 *   Tapping (merged horizontally, row 1 only) → Hand Splice, Machine Splice
 *   Issue (row 1 label) → Pressing
 *   Process Waste  (merged rows 1–2, or block)
 *   Sales          (merged rows 1–2, or block)
 *   Closing Balance (merged rows 1–2, or block)
 *
 * When includeCostAndExpense is true, every quantity column expands into a
 * 3-col sub-block: Qty | Amount | Expense Amount (header row 2 gets these labels).
 */
const GenerateTappingItemStockRegisterThicknessWiseExcel = async (
  ...
  // ── Layout math ────────────────────────────────────────────────────────
  // Base (non-cost) quantity keys, in column order.
  const KEY_COLS = 4; // Item Name, Thickness, Log No, Date
