 ...
 *
 * Layout (12 columns, 2-row header):
 *
 * Row 1 (title): "Report -1"
 * Row 2 (title): "Splicing Item Stock Register sales name - thickness wise - DD/MM/YYYY and DD/MM/YYYY"
 * Row 3: blank gap
 *
 * Header Row 1:
 ...
 *   Col  6: Opening Balance    (merged rows 1–2)
 *   Cols 7–8: Tapping          (merged horizontally, row 1 only)
 *   Col  9: Issue              (row 1 label; sub-label "Pressing" in row 2)
 *   Col 10: Process Waste      (merged rows 1–2)
 *   Col 11: Sales              (merged rows 1–2)
 *   Col 12: Closing Balance    (merged rows 1–2)
 *
 * Header Row 2:
 *   (Cols 1–6, 10–12 merged down from row 1)
 *   Col  7: Hand Splice
 *   Col  8: Machine Splice
 *   Col  9: Pressing
 *
...
const GenerateTappingStockRegisterThicknessWiseExcel = async (
  ...
  r += 2; // blank gap row between title and header

  // ─── 2-Row Header ─────────────────────────────────────────────────────────
  const hRow1 = r;
  const hRow2 = r + 1;

  // Cols merged vertically (rows 1–2): 1, 2, 3, 4, 5, 6, 10, 11, 12
  const verticalCols = [
    ...
    { col: 4,  label: 'Log No'          },
    { col: 5,  label: 'Date'            },
    { col: 6,  label: 'Opening Balance' },
    { col: 10, label: 'Process Waste'   },
    { col: 11, label: 'Sales'           },
    { col: 12, label: 'Closing Balance' },
  ];
  ...
  // "Tapping" merged across cols 7–8, row 1 only
  ws.mergeCells(hRow1, 7, hRow1, 8);
  const tappingCell = ws.getCell(hRow1, 7);
  tappingCell.value = 'Tapping';
  styleHeader(tappingCell);

  // "Issue" label in row 1 col 9 (not vertically merged — sub-label "Pressing" goes in row 2)
  const issueCell = ws.getCell(hRow1, 9);
  issueCell.value = 'Issue';
  styleHeader(issueCell);

  // Row 2 sub-labels for cols 7, 8, 9
  const subLabels = [
    { col: 7, label: 'Hand Splice'    },
    { col: 8, label: 'Machine Splice' },
    { col: 9, label: 'Pressing'       },
  ];
  ...
  rows.forEach((row) => {
    ...
    dataRow.getCell(6).value  = row.opening_balance;
    dataRow.getCell(7).value  = row.tapping_hand;
    dataRow.getCell(8).value  = row.tapping_machine;
    dataRow.getCell(9).value  = row.issue_pressing;
    dataRow.getCell(10).value = row.process_waste;
    dataRow.getCell(11).value = row.sales;
    dataRow.getCell(12).value = row.closing_balance;
  ...
  totalRow.getCell(6).value  = totOpeningBalance;
  totalRow.getCell(7).value  = totTappingHand;
  totalRow.getCell(8).value  = totTappingMachine;
  totalRow.getCell(9).value  = totIssuePressing;
  totalRow.getCell(10).value = totProcessWaste;
  totalRow.getCell(11).value = totSales;
  totalRow.getCell(12).value = totClosingBalance;
  ...
  ws.columns = [
    ...
    { width: 22 }, // Col  2: Sales Item Name
    { width: 12 }, // Col  3: Thickness
    { width: 14 }, // Col  4: Log No
    { width: 14 }, // Col  5: Date
    { width: 16 }, // Col  6: Opening Balance
    { width: 14 }, // Col  7: Hand Splice
    { width: 16 }, // Col  8: Machine Splice
    { width: 14 }, // Col  9: Pressing
    { width: 14 }, // Col 10: Process Waste
    { width: 10 }, // Col 11: Sales
    { width: 16 }, // Col 12: Closing Balance
  ];
