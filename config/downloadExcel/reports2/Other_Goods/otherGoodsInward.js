 ...
 */
const GenerateOtherGoodsInwardReport = async (details, startDate, endDate, includeCostAndExpense) => {
    ...
    currentRow++;
    const totalCols = includeCostAndExpense ? 16 : 10;
    const remarkCol = includeCostAndExpense ? 15 : 9;
    const authorisedCol = includeCostAndExpense ? 16 : 10;

    const headers = [
        { name: 'Sr. No', width: 8 },
        { name: 'Supplier Name', width: 30 },
        { name: 'Inv. No/Challan N', width: 20 },
        { name: 'Inv./Challan Date', width: 15 },
        { name: 'Item Name', width: 30 },
        { name: 'Department', width: 20 },
        { name: 'Machine', width: 20 },
        { name: 'Qty', width: 10 },
        ...(includeCostAndExpense
            ? [
                ...
                { name: 'Value', width: 15 },
                { name: 'Gst', width: 10 },
                { name: 'Total Gst', width: 12 },
            ]
            : []),
        { name: 'Remark', width: 20 },
        { name: 'Authorised', width: 15 },
    ];

    if (includeCostAndExpense) {
        // ---- Two-row header (GST group needs a sub-header) ----
        const headerRow1 = worksheet.getRow(currentRow);

        worksheet.mergeCells(currentRow, 11, currentRow, 13);
        const gstCell = worksheet.getCell(currentRow, 11);
        gstCell.value = 'Gst';
        gstCell.alignment = { horizontal: 'center', vertical: 'middle' };

        headers.forEach((header, index) => {
            const colIndex = index + 1;
            if (colIndex >= 11 && colIndex <= 13) return; // Cgst/Sgst/Igst handled below
        ...
        // ---- Single-row header (no GST group, no second row needed) ----
        const headerRow1 = worksheet.getRow(currentRow);
    ...
    details.forEach((item, index) => {
        ...
        row.getCell(remarkCol).value = item.remark || '';
        row.getCell(authorisedCol).value = item.approved_by || '';
        ...
        for (let i = 1; i <= totalCols; i++) {
            ...
            const rightAlignedCols = includeCostAndExpense
                ? [1, 8, 9, 10, 11, 12, 13, 14]
                : [1, 8];

            if (rightAlignedCols.includes(i)) {
