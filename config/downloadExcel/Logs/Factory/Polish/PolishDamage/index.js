...
export const createFactoryPolishDamageExcel = async (newData, req, res) => {
  try {
    ...
    worksheet.columns = [
      ...
      { header: 'Polish Date', key: 'polish_date', width: 18 },
    ...
    for (const item of dataArray) {
      const polishDetails = item.polish_done_details || {};
      const pressingDetails = item?.issue_for_polish_details?.pressing_details || {};
      const orderDetails = item?.issue_for_polish_details?.order_details || {};

      const pressingConsumed = item?.issue_for_polish_details?.pressing_done_consumed_items_details || [];
      ...
      worksheet.addRow({
        sr_no: item?.sr_no,
        issued_from: item?.issue_for_polish_details?.issued_from || '',
        issued_for: item?.issue_for_polish_details?.issued_for || '',
        ...
        pressing_date: pressingDetails?.pressing_date
          ...
          : '',
        polish_date: polishDetails?.polish_date
          ? new Date(polishDetails.polish_date).toLocaleDateString()
          : '',
        issued_sheets: item?.issue_for_polish_details?.issued_sheets || '',
        issued_sqm: item?.issue_for_polish_details?.issued_sqm || '',
    ...
    res.setHeader('Content-Disposition', 'attachment; filename=Factory-Polish-Damage-Report.xlsx');
