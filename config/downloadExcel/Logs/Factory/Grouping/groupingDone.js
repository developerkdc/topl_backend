...
export const createFactoryGroupingDoneExcel = async (details, req, res) => {
  try {
    const workbook = new exceljs.Workbook();
    const worksheet = workbook.addWorksheet('Done Report');
    ...
    const headers = [
      ...
      { header: 'No Of Leaves', key: 'no_of_leaves', width: 15 },
      {
        header: 'Available No Of Leaves',
        key: 'available_no_of_leaves',
        width: 20,
      },
    ...
    details.forEach((item) => {
      ...
      const row = worksheet.addRow({
        ...
        available_no_of_leaves: item?.available_details?.no_of_leaves ?? '',
        sqm: item?.sqm || '',
    ...
    const fileName = `Grouping_Done_Report_${timestamp}.xlsx`;
