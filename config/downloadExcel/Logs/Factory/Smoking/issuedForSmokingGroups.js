...
const GenerateIssuedForSmokingGroupsLogs = async (details) => {
  const workbook = new ExcelJS.Workbook();
  const worksheet = workbook.addWorksheet("Issued For Smoking Groups Logs");
  ...
  details.forEach((log) => {
    ...
    fullDocument.item_details.forEach((item, index) => {
      const row = worksheet.addRow({
        ...
        created_by: index === 0 ? `${log.created_user_id.first_name} ${log.created_user_id.last_name} ` : "",
        date_of_grouping:
...
export { GenerateIssuedForSmokingGroupsLogs };
