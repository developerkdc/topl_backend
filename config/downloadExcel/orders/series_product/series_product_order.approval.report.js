...
export const create_series_product_order_approval_report = async (newData, req, res) => {
    try {
        const workbook = new exceljs.Workbook();
        const worksheet = workbook.addWorksheet('Series-Product-Order-Approval-Logs');

        worksheet.columns = [
            { header: 'Sr. No', key: 'sr_no', width: 8 },
            { header: 'Order No', key: 'order_no', width: 14 },
            { header: 'Order Date', key: 'order_date', width: 15 },
            { header: 'Approval For', key: 'approval_for', width: 18 },
        ...
        Object.values(newData).forEach((item, index) => {
            ...
            worksheet.addRow({
                ...
                approval_for: order.approval_for?.split("_")?.join(" ") || '-',
        ...
        const fileName = `Series_Product_Order_Approval_Logs_${Date.now()}.xlsx`;
        res.setHeader(
            'Content-Type',
