...
const GenerateIssueFinishingReport = async (details) => {

    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet("Issue For Finishing Reports");
    ...
    const headers = [
        ...
        { header: "Original Pressing Pcs", key: "pressing_no_of_peices", width: 15 },
        { header: "Pressing Sqm", key: "pressing_sqm", width: 15 },
        { header: "Avl Pressing Pcs", key: "available_pressed_pcs", width: 15 },
    ...
    details.forEach((finishing) => {
        const itemData = finishing?.cutting_details?.cutting_id?.[0]?.item_details?.item_data?.[0]
        const row = worksheet.addRow({
            created_at: convDate(finishing?.created_at),
            group_no: finishing?.ready_sheet_form_history_details?.group_no,
            item_name: itemData?.item_name,
            item_code: itemData?.item_code,
            pressing_length: finishing?.pressing_details?.pressing_length,
            pressing_width: finishing?.pressing_details?.pressing_width,
            pressing_no_of_peices: finishing?.pressing_details?.pressing_no_of_peices,
            pressing_sqm: finishing?.pressing_details?.pressing_sqm,
            available_pressed_pcs: finishing?.available_pressed_pcs,
            status: finishing?.status,

            // ready_sheet_form_length: finishing?.ready_sheet_form_history_details?.ready_sheet_form_length,
            // ready_sheet_form_width: finishing?.ready_sheet_form_history_details?.ready_sheet_form_width,
            // ready_sheet_form_approved_pcs: finishing?.ready_sheet_form_history_details?.ready_sheet_form_approved_pcs,
            // ready_sheet_form_sqm: finishing?.ready_sheet_form_history_details?.ready_sheet_form_sqm,
            // ready_sheet_form_pallete_no: finishing?.ready_sheet_form_history_details?.ready_sheet_form_pallete_no,
            // ready_sheet_form_physical_location: finishing?.ready_sheet_form_history_details?.ready_sheet_form_physical_location,
        ...
        // finishing?.cutting_details?.cutting_id?.forEach((item) => {
    ...
    const filePath = "public/reports/Finishing/IssueForFinishingReportExcel/issue_for_finishing_report.xlsx";
    ...
    const downloadFileName = `issue_for_finishing_report_${timestamp}.xlsx`;

    // Move the file to the desired folder
    const destinationPath = `public/reports/Finishing/IssueForFinishingReportExcel/${downloadFileName}`;
...
export { GenerateIssueFinishingReport };
