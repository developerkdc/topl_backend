...
const GenerateIssueFinishingReport = async (details) => {
  ...
  const headers = [
    ...
    {
      header: 'Original Pressing Pcs',
      key: 'pressing_no_of_peices',
      width: 15,
    },
  ...
  details.forEach((finishing) => {
    const itemData =
      finishing?.cutting_details?.cutting_id?.[0]?.item_details?.item_data?.[0];
  ...
  const filePath =
    'public/reports/Finishing/IssueForFinishingReportExcel/issue_for_finishing_report.xlsx';
