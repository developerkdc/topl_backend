...
const GenerateIssuePressingReport = async (details) => {
  ...
  const headers = [
    ...
    {
      header: 'Ready Sheet Pcs',
      key: 'ready_sheet_form_approved_pcs',
      width: 15,
    },
    { header: 'Ready Sheet Sqm', key: 'ready_sheet_form_sqm', width: 15 },
    {
      header: 'Ready Sheet Pallet No',
      key: 'ready_sheet_form_pallete_no',
      width: 15,
    },
    {
      header: 'Ready Sheet Physical Location',
  ...
  details.forEach((pressing) => {
    const itemData =
      pressing?.cutting_details?.cutting_id?.[0]?.item_details?.item_data?.[0];
    const row = worksheet.addRow({
      ...
      ready_sheet_form_length:
        pressing?.ready_sheet_form_history_details?.ready_sheet_form_length,
      ready_sheet_form_width:
        pressing?.ready_sheet_form_history_details?.ready_sheet_form_width,
      ready_sheet_form_approved_pcs:
        pressing?.ready_sheet_form_history_details
          ?.ready_sheet_form_approved_pcs,
      ready_sheet_form_sqm:
        pressing?.ready_sheet_form_history_details?.ready_sheet_form_sqm,
      ready_sheet_form_pallete_no:
        pressing?.ready_sheet_form_history_details?.ready_sheet_form_pallete_no,
  ...
  const filePath =
    'public/reports/Pressing/IssueForPressingReportExcel/issue_for_pressing_report.xlsx';
