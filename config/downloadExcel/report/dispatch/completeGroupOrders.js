...
const GenerateCompleteGroupOrdersReport = async (details) => {
  const workbook = new ExcelJS.Workbook();
  const worksheet = workbook.addWorksheet("Complete Group Orders Reports");
  ...
  const filePath =
    "public/reports/Dispatch/CompleteGroupOrdersReportExcel/complete_group_orders.xlsx";
  await workbook.xlsx.writeFile(filePath);

  const timestamp = new Date().getTime();
  const downloadFileName = `Complete_Group_Orders_Report_${timestamp}.xlsx`;

  const destinationPath = `public/reports/Dispatch/CompleteGroupOrdersReportExcel/${downloadFileName}`;
...
export { GenerateCompleteGroupOrdersReport };
