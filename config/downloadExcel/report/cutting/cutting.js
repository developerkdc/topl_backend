...
const GenerateCuttingReport = async (details) => {
    ...
    const headers = [
        ...
        { header: "Group No of Pattas", key: "group_no_of_pattas_available", width: 15 },
    ...
    details.forEach((cutting) => {
        cutting?.group_history_id?.cutting_item_details?.forEach((itemData,index) => {
            const row = worksheet.addRow({
                ...
                wastage_sqm: cutting?.item_details?.[index]?.waste_cutting_quantity?.waste_sqm,
                waste_sqm_percentage: cutting?.item_details?.[index]?.waste_cutting_quantity?.waste_sqm_percentage,
